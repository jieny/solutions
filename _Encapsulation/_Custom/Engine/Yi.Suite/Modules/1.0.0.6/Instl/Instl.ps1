		...
		Switch ($mode)
		{
			Fast {
				if ([string]::IsNullOrEmpty($param))
				{
					...
					Write-Host "    - $($lang.Parameter)" -ForegroundColor Yellow
...
Function Update_Process
{
	...
	if ($ServerTest) {
		...
		$output = "$($PSScriptRoot)\Backup\$($Global:IsLang)\latest.json"
...
<#
	.初始化默认配置文件
#>
$Script:Init_Config = "$($PSScriptRoot)\$($Global:IsLang)\latest.json"

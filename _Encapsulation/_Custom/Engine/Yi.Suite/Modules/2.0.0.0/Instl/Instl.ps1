					...
					if (Test-Path $OutAny) {
						...
						if ([string]::IsNullOrEmpty($url)) {
							...
							if (Test_URI $url) {
								...
								Invoke-WebRequest -Uri $url -OutFile $OutAny -UseBasicParsing -ErrorAction SilentlyContinue | Out-Null
							} else {
...
Function Update_Process
{
	...
	if ($ServerTest) {
		...
		Invoke-WebRequest -Uri $url -OutFile $output -TimeoutSec 30 -UseBasicParsing -DisableKeepAlive -ErrorAction SilentlyContinue | Out-Null

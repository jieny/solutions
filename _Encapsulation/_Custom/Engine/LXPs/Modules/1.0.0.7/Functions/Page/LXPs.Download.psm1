...
Function LXPs_Download
{
	...
	Function LXPs_Refresh_Sources_To_Event
	{
		...
		if ($UI_Main_Download.Checked) {
			...
			$UI_Main_Save_To.Text = Join-Path -Path $GetCurrentDisk -ChildPath "Download\Default" -ErrorAction SilentlyContinue
		} else {
			...
			if ($UI_Main_Sync_Some_Location.Checked) {
				if ([string]::IsNullOrEmpty($UI_Main_Download_Match_Version_Select.Text)) {
					$UI_Main_Save_To.Text = Join-Path -Path $GetCurrentDisk -ChildPath "Download\$($RandomGuid)" -ErrorAction SilentlyContinue
				} else {
					$UI_Main_Save_To.Text = Join-Path -Path $GetCurrentDisk -ChildPath "Download\$($UI_Main_Download_Match_Version_Select.Text)" -ErrorAction SilentlyContinue
				}
			} else {
				<#
	...
	Function LXPs_Refresh_Sources_To_Status
	{
		...
		if (Test-Path -Path $UI_Main_Mask_Report_Sources_Path.Text -PathType Container) {
			if (Test_Available_Disk -Path $UI_Main_Mask_Report_Sources_Path.Text) {
				...
				$UI_Main_Mask_Report_Save_To.Text = Join-Path -Path $DesktopOldpath -ChildPath "Report.$($RandomGuid).csv" -ErrorAction SilentlyContinue
			} else {
	...
	$UI_Main_Match_No_Select_Item = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			...
			$TestNewFolderPathLep = Join-Path -Path $UI_Main_Save_To.Text -ChildPath "LocalExperiencePack" -ErrorAction SilentlyContinue
			...
			if (Test-Path -Path $FolderDirect -PathType Container) {
				...
				ForEach ($itemRegion in $Region) {
					$TempNewFileFullPath = Join-Path -Path $FolderDirect -ChildPath "$($itemRegion.Region)\LanguageExperiencePack.$($itemRegion.Region).Neutral.appx" -ErrorAction SilentlyContinue
	...
	$UI_Main_Mask_Report_Sources_Select_Folder = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				...
				if (Test-Path -Path $FolderBrowser.SelectedPath -PathType Container) {
					if (Test_Available_Disk -Path $FolderBrowser.SelectedPath) {
						$UI_Main_Mask_Report_Save_To.Text = Join-Path -Path $FolderBrowser.SelectedPath -ChildPath "Report.$($RandomGuid).csv" -ErrorAction SilentlyContinue
					} else {
						$UI_Main_Mask_Report_Save_To.Text = Join-Path -Path $DesktopOldpath -ChildPath "Report.$($RandomGuid).csv" -ErrorAction SilentlyContinue
					}
			...
			if (-not [string]::IsNullOrEmpty($UI_Main_Save_To.Text)) {
				...
				if (Test-Path -Path $UI_Main_Save_To.Text -PathType Container) {
					if (Test_Available_Disk -Path $UI_Main_Save_To.Text) {
						$UI_Main_Mask_Report_Save_To.Text = Join-Path -Path $UI_Main_Save_To.Text -ChildPath "Report.$($RandomGuid).csv" -ErrorAction SilentlyContinue
					} else {
						$UI_Main_Mask_Report_Save_To.Text = Join-Path -Path $DesktopOldpath -ChildPath "Report.$($RandomGuid).csv" -ErrorAction SilentlyContinue
					}
				} else {
...
Function LXPs_Download_Report_Process
{
	...
	$NewFolderLEP = Join-Path -Path $Path -ChildPath "LocalExperiencePack" -ErrorAction SilentlyContinue
	...
	$Region = Language_Region
	ForEach ($itemRegion in $Region) {
		$TempNewFileFolderPath = Join-Path -Path $ISOTestFolderMain -ChildPath $itemRegion.Region -ErrorAction SilentlyContinue
		$TempNewFileFullPath = Join-Path -Path $FolderDirect -ChildPath "$($itemRegion.Region)\LanguageExperiencePack.$($itemRegion.Region).Neutral.appx" -ErrorAction SilentlyContinue
		...
		if (Test-Path -Path $TempNewFileFullPath -PathType Leaf) {
			...
			$zipFile.Entries | where { $_.Name -like 'AppxManifest.xml' } | ForEach {
				[System.IO.Compression.ZipFileExtensions]::ExtractToFile($_, $(Join-Path -Path $TempNewFileFolderPath -ChildPath $_.Name -ErrorAction SilentlyContinue), $true)
			}
			$zipFile.Dispose()

			$TempNewFileAppxManifest = Join-Path -Path $TempNewFileFolderPath -ChildPath "AppxManifest.xml" -ErrorAction SilentlyContinue
...
Function LXPs_Download_Licence_Process
{
	...
	$Region = Language_Region
	ForEach ($itemRegion in $Region) {
		$TempNewFileFolderPath = Join-Path -Path $FolderDirect -ChildPath $itemRegion.Region -ErrorAction SilentlyContinue
		$TempNewFileFullPath = Join-Path -Path $FolderDirect -ChildPath "$($itemRegion.Region)\LanguageExperiencePack.$($itemRegion.Region).Neutral.appx" -ErrorAction SilentlyContinue
	...
	if ($QueueLXPsLicenceSelect.count -gt 0) {
		...
		Write-host "   $('-' * 80)"
		ForEach ($item in $QueueLXPsLicenceSelect) {
			$TempNewFileFullPath = Join-Path -Path $item.OrgPath -ChildPath $item.FileName -ErrorAction SilentlyContinue

			if (Test-Path -Path $TempNewFileFullPath -PathType Leaf) {
				$Wait_Remove_Temp_File = Join-Path -Path $item.OrgPath -ChildPath "License.xml" -ErrorAction SilentlyContinue
				...
				$zipFile.Entries | where { $_.Name -like 'License.xml' } | ForEach {
					[System.IO.Compression.ZipFileExtensions]::ExtractToFile($_, $(Join-Path -Path $item.OrgPath -ChildPath $_.Name -ErrorAction SilentlyContinue), $true)
				}
				$zipFile.Dispose()
	...
	if ($Script:Queue_Language_Download_Select.Count -gt 0) {
		...
		ForEach ($itemRegion in $Region) {
			if ($Script:Queue_Language_Download_Select -Contains $itemRegion.Region) {
				$NewFolder = Join-Path -Path $UI_Main_Save_To.Text -ChildPath "LocalExperiencePack\$($itemRegion.Region)" -ErrorAction SilentlyContinue
				...
				if ($Script:IsDownload) {
					...
					$test_new_Filename = Join-Path -Path $NewFolder -ChildPath $NewFilename -ErrorAction SilentlyContinue
...
#		This works when Office is not installed
	...
	{
		$InitalSaveToDownload = Join-Path -Path $SaveTo -ChildPath $Download.textContent -ErrorAction SilentlyContinue
		...
		if ($Script:IsDownload) {
			...
			if ($Download.textContent -like "*$($Script:Version)*.appx") {
				...
				if (Test-Path -Path $InitalSaveToDownload -PathType Leaf) {
					$SaveToNewFileLep = Join-Path -Path $SaveTo -ChildPath "LanguageExperiencePack.$($NewLang).Neutral.appx" -ErrorAction SilentlyContinue
					Write-Host "   $($lang.Done)" -ForegroundColor Green
					...
					if ($Script:IsLicence) {
						...
						$NewFileLepLicense = Join-Path -Path $SaveTo -ChildPath "License.xml" -ErrorAction SilentlyContinue
						...
						if (Test-Path -Path $TempNewFileFullPath -PathType Leaf) {
							...
							try {
								...
								$zipFile = [IO.Compression.ZipFile]::OpenRead($TempNewFileFullPath)
								$zipFile.Entries | where { $_.Name -like 'License.xml' } | ForEach {
									[System.IO.Compression.ZipFileExtensions]::ExtractToFile($_, $(Join-Path -Path $SaveTo -ChildPath $_.Name -ErrorAction SilentlyContinue), $true)
								}
								$zipFile.Dispose()

...
Function Installation_interface_UI
{
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Author)\Multilingual\Get" -Name "InstlTo" -ErrorAction SilentlyContinue) {
		$UI_Main_Save_To_Path.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Author)\Multilingual\Get" -Name "InstlTo" -ErrorAction SilentlyContinue
	} else {

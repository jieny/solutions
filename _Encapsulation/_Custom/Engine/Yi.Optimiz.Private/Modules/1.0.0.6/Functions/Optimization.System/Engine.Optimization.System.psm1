...
Function Optimization_System_UI
{
	...
	Write-Host "   $($lang.Optimize) $($lang.System)`n   $('-' * 80)"
...
Function Taskbar_Suggested_Content
{
	...
	if ($Show) {
		...
		ForEach ($item in $cdm) {
			New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager' -Name $item -Value 1 -PropertyType DWord -Force -ea SilentlyContinue | Out-Null
		}
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
	...
	if ($Hide) {
		...
		New-Item -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Force -ErrorAction SilentlyContinue | Out-Null
		ForEach ($item in $cdm) {
			New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager' -Name $item -Value 0 -PropertyType DWord -Force -ea SilentlyContinue | Out-Null
		}
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}

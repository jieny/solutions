...
Function UWP_Uninstall
{
	...
	Write-Host "   $($lang.Delete) $($lang.UninstallUWP)`n   $('-' * 80)"
	...
	$UI_Main_OK        = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Close_Store_Auto_Download.Checked) {
				...
				New-ItemProperty -LiteralPath 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsStore' -Name 'AutoDownload' -Value 2 -PropertyType DWord -Force -ea SilentlyContinue | Out-Null
				Write-Host "$($lang.Done)`n" -ForegroundColor Green
			} else {
				Write-Host "   $($lang.Inoperable)`n" -ForegroundColor Red
			}
			...
			if ($UI_Main_Suggestions_Apps.Checked) {
				...
				New-ItemProperty -LiteralPath 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent' -Name 'DisableWindowsConsumerFeatures' -Value 1 -PropertyType DWord -Force -ea SilentlyContinue | Out-Null
				Write-Host "$($lang.Done)`n" -ForegroundColor Green
			} else {
				Write-Host "   $($lang.Inoperable)`n" -ForegroundColor Red
			}

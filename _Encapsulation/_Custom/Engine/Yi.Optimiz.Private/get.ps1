<#
	...
	.Learn
	 Interactive installation
	    https://github.com/ilikeyi/Yi.Optimiz.Private/blob/main/_Learn/Get/Get.pdf
...
$Author = "Yi"
$Default_directory_name = "Yi.Optimiz.Private"
$Update_Server = @(
...
Function Installation_interface_UI
{
		...
		add_Click      = {
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				if (Test_Available_Disk -Path $FolderBrowser.SelectedPath) {
					...
					Save_Dynamic -regkey "Yi.Optimiz.Private\Get" -name "InstlTo" -value $UI_Main_Save_To_Path.Text -String
				} else {
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Author)\Yi.Optimiz.Private\Get" -Name "InstlTo" -ErrorAction SilentlyContinue) {
		$UI_Main_Save_To_Path.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Author)\Yi.Optimiz.Private\Get" -Name "InstlTo" -ErrorAction SilentlyContinue
	} else {

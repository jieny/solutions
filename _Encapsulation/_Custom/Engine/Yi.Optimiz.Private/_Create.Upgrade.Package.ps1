...
Function Update_Create_UI
{
   ...
   buildstring: $((Get-Module -Name Engine).Version.ToString()).bs_release.2024.12.08`n"
			...
			if ($UI_Main_Create_ASC.Enabled) {
				if ($UI_Main_Create_ASC.Checked) {
					if ([string]::IsNullOrEmpty($UI_Main_Create_ASCSign.Text)) {
						...
						Save_Dynamic -regkey "Yi.Optimiz.Private" -name "PGP" -value $UI_Main_Create_ASCSign.Text -String
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Engine).Author)\Yi.Optimiz.Private" -Name "PGP" -ErrorAction SilentlyContinue) {
		$UI_Main_Create_ASCSign.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Engine).Author)\Yi.Optimiz.Private" -Name "PGP" -ErrorAction SilentlyContinue
	}

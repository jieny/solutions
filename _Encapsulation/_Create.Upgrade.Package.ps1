...
<#
  ...
  .压缩包保存到
#>
$UpdateSaveTo = "$([Environment]::GetFolderPath("Desktop"))\Solutions.Upgrade.Package"

<#
	.Archive temporary directory
	.压缩包临时目录
#>
$TempFolderUpdate = "$([Environment]::GetFolderPath("MyDocuments"))\Temp.Solutions.Upgrade.Package"
...
$ArchiveExcludeUp = @(
	...
	"-xr-!_Custom\Software"
	"-xr-!_Custom\Fonts"
	"-xr-!_Custom\Office\setup.exe"
	"-xr-!_Custom\Office\2024\amd64\Office\Data"
	"-xr-!_Custom\Office\2024\x86\Office\Data"

	"-xr-!_Custom\Office\365\amd64\Office\Data"
	"-xr-!_Custom\Office\365\x86\Office\Data"

	"-xr-!_Custom\Office\2021\amd64\Office\Data"
	"-xr-!_Custom\Office\2021\x86\Office\Data"

	"-xr-!_Custom\Office\365\2019\Office\Data"
	"-xr-!_Custom\Office\365\2019\Office\Data"

	"-xr-!_Custom\Engine\LXPs\Download"
	"-xr-!_Custom\Engine\LXPs\Logs"
	"-xr-!_Custom\Engine\Multilingual\LXPs\Deploy"
	"-xr-!_Custom\Engine\Multilingual\LXPs\Logs"
	"-xr-!_Custom\Engine\Yi.Optimiz.Private\LXPs\Deploy"
	"-xr-!_Custom\Engine\Yi.Optimiz.Private\LXPs\Logs"
...
Function Get_Zip
{
	...
	$Local_Zip_Path = @(
		...
		"$(Get_Arch_Path -Path "$($PSScriptRoot)\Modules\AIO\7zPacker")\$($Run)"
	)
...
Function Update_Create_UI
{
	...
	Clear-Host
	$Host.UI.RawUI.WindowTitle = "$((Get-Module -Name Solutions).Author)'s Solutions | $($lang.UpdateCreate)"
   ...
   buildstring: $((Get-Module -Name Solutions).Version.ToString()).bs_release.2024.04.18`n"
	...
	$GUIUpdateLowVersion = New-Object system.Windows.Forms.Label -Property @{
		...
		Height         = 30
		Width          = 390
		Text           = "$($lang.UpdateLow) $((Get-Module -Name Solutions).PrivateData.PSData.MinimumVersion)"
	}
...
Function Update_Create_Process
{
	ForEach ($item in $BuildTypeUp) {
		...
		Update_Create_Version -SaveTo "$TempFolderUpdate" -CurrentVersion (Get-Module -Name Solutions).Version.ToString() -LowVer $((Get-Module -Name Solutions).PrivateData.PSData.MinimumVersion)
	}
}

	...
	Function Wimlib_Image_Refresh_Details_Rule
	{
		...
		ForEach ($item in $Global:Primary_Key_Image.Index) {
			$CheckBox   = New-Object System.Windows.Forms.RadioButton -Property @{
				Height  = 105
				Width   = 528
				Text    = "$($lang.Wim_Image_Name): $($item.ImageName)`n$($lang.Wim_Image_Description): $($item.ImageDescription)`n$($lang.Wim_Display_Name): $($item.DISPLAYNAME)`n$($lang.Wim_Display_Description): $($item.DISPLAYDESCRIPTION)`n$($lang.MountedIndex): $($item.ImageIndex)"
				Tag     = $item.ImageIndex
				add_Click = {
					$UI_Main_Error.Text = ""
					$UI_Main_Error_Icon.Image = $null

...
$($getSerVer.changelog.log)`n"
			...
			Save_Dynamic -regkey "Solutions\Update" -name "Auto_Update_Last_status" -value "$($lang.Auto_Update_IsLatest -f $($Global:Author))"
			Write-Host "  $($lang.UpdateNoUpdateAvailable -f $($Global:Author))"
		}
	} else {
		$NewErrorTips = "$($lang.UpdateNotSatisfied -f $((Get-Module -Name Solutions).PrivateData.PSData.MinimumVersion), $($Global:Author))"

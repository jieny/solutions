...
Function UnPack_Create_UI
{
	...
	Function Unpack_API_Add_New_Rule_Name
	{
		...
		if (Unpack_API_Verify_New_RuleName) {
			...
			New-Item "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Unpack\API\Custom\$($Unpack_API_Rule_Path.Text)" -force -ErrorAction SilentlyContinue | Out-Null
			New-ItemProperty -LiteralPath "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Unpack\API\Custom\$($Unpack_API_Rule_Path.Text)" -Name "Path" -Value $Unpack_API_New_Path.Text -PropertyType String -force -ErrorAction SilentlyContinue | Out-Null
			...
			ForEach ($item in $GetALlName) {
				...
				$CheckboxChange    = New-Object system.Windows.Forms.LinkLabel -Property @{
					...
					add_Click      = {
						...
						$Unpack_API_New_Path.Text = $this.Tag
					}
				}
	...
	If (Test-Path -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Unpack\API\Custom") {
		...
		New-Item "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Unpack\API\Custom\$($Global:Author)" -force -ErrorAction SilentlyContinue | Out-Null
		$InitPath = Join-Path -Path $([Environment]::GetFolderPath("Desktop")) -ChildPath "$($Global:Author).ps1"
		New-ItemProperty -LiteralPath "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Unpack\API\Custom\$($Global:Author)" -Name "Path" -Value $InitPath -PropertyType String -force -ErrorAction SilentlyContinue | Out-Null
	}

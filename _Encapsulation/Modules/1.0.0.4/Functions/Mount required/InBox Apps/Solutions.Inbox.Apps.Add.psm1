...
Function InBox_Apps_Add_UI
{
	...
	Function Autopilot_InBox_Apps_Add_UI_Save
	{
		...
		if (InBox_Apps_Check_Customize -RuleNaming -SelectApps -AddSources) {
			...
			Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -name "$(Get_GPS_Location)_UWPExclusions" -value $Temp_InBox_Appx_User_No_Select -Multi
				...
				Get-CimInstance -Class Win32_LogicalDisk -ErrorAction SilentlyContinue | Where-Object { -not ([string]::IsNullOrEmpty($_) -or [string]::IsNullOrWhiteSpace($_))} | ForEach-Object {
					...
					if (Test-Path $SearchTempFile -PathType Leaf) {
						...
						try {
							...
							Add-AppxProvisionedPackage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Add-AppxProvisionedPackage.log" -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -PackagePath "$($SearchTempFile)" -SkipLicense $CommandIsWaitInstallRegions -ErrorAction SilentlyContinue | Out-Null
							Write-Host "   $($lang.Done)" -ForegroundColor Green
						} catch {
							Write-Host "   $($lang.SelectFromError)" -ForegroundColor Red
							Write-Host "   $($_)" -ForegroundColor Yellow
	...
	if ($Script:GroupExcludePreInstall -contains $Name) {
		if (Test-Path -Path $AppxSource -PathType Leaf) {
			...
			if ([string]::IsNullOrEmpty($CertificateSource)) {
				...
				$CommandNew = "Add-AppxProvisionedPackage -ScratchDirectory ""$(Get_Mount_To_Temp)"" -LogPath ""$(Get_Mount_To_Logs)\Add-AppxProvisionedPackage.log"" -Path ""$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"" -PackagePath ""$($AppxSource)"" -SkipLicense $($CommandIsWaitInstallRegions) $($CommandIsWaitInstallDependencyPackage) -ErrorAction SilentlyContinue | Out-Null"
				$CommandNewPrint = "Add-AppxProvisionedPackage -Path ""$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"" -PackagePath ""$($AppxSource)"" -SkipLicense $($CommandIsWaitInstallRegions) $($CommandIsWaitInstallDependencyPackage)"
				...
				if (Test-Path -Path $CertificateSource -PathType Leaf) {
					...
					$CommandNew = "Add-AppxProvisionedPackage -ScratchDirectory ""$(Get_Mount_To_Temp)"" -LogPath ""$(Get_Mount_To_Logs)\Add-AppxProvisionedPackage.log"" -Path ""$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"" -PackagePath ""$($AppxSource)"" -LicensePath ""$($CertificateSource)"" $($CommandIsWaitInstallRegions) $($CommandIsWaitInstallDependencyPackage) -ErrorAction SilentlyContinue | Out-Null"
					$CommandNewPrint = "Add-AppxProvisionedPackage -Path ""$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"" -PackagePath ""$($AppxSource)"" -LicensePath ""$($CertificateSource)"" $($CommandIsWaitInstallRegions) $($CommandIsWaitInstallDependencyPackage)"

...
Function InBox_Apps_Mark_UI
{
	...
	Function Autopilot_InBox_Apps_Mark_UI_Save
	{
		...
		if ($UI_Main_Is_Install_LXPs.Checked) {
			New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $True -Force
			New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Custom_Select_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value @() -Force
			...
			if ($UI_Main_InBox_Apps_Clear.Checked) {
				...
				New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $false -Force
			...
			if ($Temp_Select_Experience_Pack_Queue.count -gt 0) {
				New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $True -Force
				New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Custom_Select_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $Temp_Select_Experience_Pack_Queue -Force
				New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Select_Sources_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $Get_Temp_Select_Update_Add_Folder -Force
	...
	{
		$Temp_Assign_Task_Select = (Get-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Custom_Select_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value
	...
	{
		$Temp_Assign_Task_Select = (Get-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Select_Sources_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value
		$Temp_Assign_Task_Select = $Temp_Assign_Task_Select | Where-Object { -not ([string]::IsNullOrEmpty($_) -or [string]::IsNullOrWhiteSpace($_))} | Select-Object -Unique
		...
		ForEach ($itemMain in $Search_Folder_Multistage_Rule) {
			...
			if ($MarkIsFolderRule) {
				Get-ChildItem -Path $item -Directory -ErrorAction SilentlyContinue | Where-Object {
					...
					if (Test-Path $EnginePath -PathType Leaf) {
		...
		add_Click      = {
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				if ($Get_Temp_Select_Update_Add_Folder -Contains $FolderBrowser.SelectedPath) {
					...
					New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Select_Sources_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $FolderBrowser.SelectedPath -Force
					Refresh_LXPs_Engine_Local_Sources
					Refres_Event_Tasks_InBox_Apps_Region_Add_UI
	...
	$UI_Main_Save      = New-Object system.Windows.Forms.Button -Property @{
		...
		Text           = $lang.Save
		add_Click      = {
			if (Autopilot_InBox_Apps_Mark_UI_Save) {
				...
				ForEach ($item in $InitlUWPPrePakcage) {
					if ($InitlUWPPrePakcageExclude -notContains $item) {
						...
						if ((Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -ErrorAction SilentlyContinue).'ShowCommand' -eq "True") {
							Write-Host "`n   $($lang.Command), $($lang.Developers_Mode_Location)57" -ForegroundColor Green
							Write-host "   $('-' * 80)"
...
{
	$Temp_Select_Queue_LXPs_Add_Custom_Select = (Get-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Custom_Select_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value
...
Function InBox_Apps_Add_Mark_Process
{
	...
	Get-ChildItem "$($Path)\LanguageExperiencePack.*.appx" -ErrorAction SilentlyContinue | ForEach-Object {
		...
		if (Test-Path -Path "$($Path)\License.xml" -PathType Leaf) {
			...
			Add-AppxProvisionedPackage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\AppxProvisionedPackage.log" -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -PackagePath $_.FullName -LicensePath "$($Path)\License.xml" -Regions "All" -ErrorAction SilentlyContinue | Out-Null
			Write-Host $lang.Done -ForegroundColor Green
			...
			Add-AppxProvisionedPackage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Add-AppxProvisionedPackage.log" -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -PackagePath $_.FullName -SkipLicense -Regions "All" -ErrorAction SilentlyContinue | Out-Null
			Write-Host $lang.Done -ForegroundColor Green

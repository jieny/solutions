...
Function Language_Add_File_Type_Process
{
	...
	if ($Script:Init_Exclude_File -contains $FileName) {
		...
		$CommandNew = "Add-WindowsPackage -ScratchDirectory ""$(Get_Mount_To_Temp)"" -LogPath ""$(Get_Mount_To_Logs)\Add.log"" -Path ""$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"" -PackagePath ""$($FileName)"" -ErrorAction SilentlyContinue | Out-Null"
		$CommandNewPrint = "Add-WindowsPackage -Path ""$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"" -PackagePath ""$($FileName)"""

	...
	Function Autopilot_ISO_Save
	{
		...
		if ($UI_Main_Is_Create_ISO.Checked) {
			...
			if ($FlagCheckISOName) {
				...
				if (Test-Path $Global:ISOSaveToFullName -PathType Leaf) {
					<#
						.清除 ISO 临时测试文件
					#>
					Remove_Tree $ISOTestFolderMain
					Remove-Item $Global:ISOSaveToFullName -Force -ErrorAction SilentlyContinue
				...
				if (Test-Path $ISOTestFolderMain -PathType Container) {
					<#
						...
						.创建目录成功，创建一个空文件
					#>
					Out-File -FilePath "$($Global:ISOSaveToFolder)$($RandomGuid)\writetest-$($RandomGuid)" -Encoding utf8 -ErrorAction SilentlyContinue

					$OSCDIMGArch = "$(Get_Arch_Path -Path "$($PSScriptRoot)\..\..\..\..\AIO\Oscdimg")\oscdimg.exe"
					if (Test-Path $OSCDIMGArch -PathType Leaf) {
						Start-Process $OSCDIMGArch -ArgumentList "-n -d -m ""$($Global:ISOSaveToFolder)$($RandomGuid)"" ""$($Global:ISOSaveToFullName)""" -wait -WindowStyle Hidden
					}
...
Function ISO_Local_Language_Calc
{
	...
	ForEach ($itemRegion in $Region) {
		...
		if (Test-Path -Path "$($Global:Image_source)\sources\$($itemRegion.Region)" -PathType Container) {
			$NewSearchFileRule = @(
				"setup.exe.mui"
				"dism.exe.mui"
				"winsetup.dll.mui"
			...
			foreach ($NewSearchItem in $NewSearchFileRule) {
				if (Test-Path -Path "$($Global:Image_source)\sources\$($itemRegion.Region)\$($NewSearchItem)" -PathType leaf) {
#					write-host "   $($Global:Image_source)\sources\$($itemRegion.Region)\$($NewSearchItem), $($NewSearchFileCalc)" -ForegroundColor Yellow
					$NewSearchFileCalc++
				}
			}
...
Function ISO_Create_Process
{
	...
	if ($Global:Queue_ISO) {
		...
		if (Test-Path $Global:ISOSaveToFolder -PathType Container) {
			if (Test-Path $Global:ISOSaveToFullName -PathType Leaf) {
			...
			if (Test-Path $OSCDIMGArch -PathType Leaf) {
				...
				start-process $OSCDIMGArch -ArgumentList "-m -o -u2 -udfver102 -l$($Global:ISOISOCustomLabel) -bootdata:2#p0,e,b""$($Global:Image_source)\boot\etfsboot.com""#pEF,e,b""$($Global:Image_source)\efi\microsoft\boot\efisys.bin"" ""$($Global:Image_source)"" ""$($Global:ISOSaveToFullName)""" -wait -nonewwindow
			}

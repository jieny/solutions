							...
							ForEach ($item in $Global:Primary_Key_Image.Index) {
								if ($item.ImageIndex -eq $_.Tag) {
									...
									if (Test-Path -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -PathType Container) {
										...
										Repair-WindowsImage -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -RestoreHealth -ErrorAction SilentlyContinue | Out-Null
										...
										Remove_Tree "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"
									}
									...
									try {
										Mount-WindowsImage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Mount.log" -ImagePath "$($Global:Primary_Key_Image.FullPath)" -Index "$($_.Tag)" -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"
										Write-Host $lang.Done -ForegroundColor Green
									} catch {
										Write-Host $_

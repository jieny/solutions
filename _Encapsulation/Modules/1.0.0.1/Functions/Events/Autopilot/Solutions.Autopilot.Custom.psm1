	...
	Function Autopilot_Rule_Details_View
	{
		...
		if ($Script:InBox_Apps_Rule_Select_Single.count -gt 0) {
			...
			if ($Script:InBox_Apps_Rule_Select_Single.Autopilot.Prerequisite.Count -gt 0) {
				$UI_Main_Autopilot_View_Detailed_Show.Text += "         x64`n"
				...
				$UI_Main_Autopilot_View_Detailed_Show.Text += "`n         arm64`n"
...
#			Refresh_Config_Rule_Sync_To_Show
	...
	Function Refresh_All_Rule_Guid
	{
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "DiskTo" -ErrorAction SilentlyContinue) {
			$NewSources_ISO = "$(Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "DiskTo" -ErrorAction SilentlyContinue)\$($Global:Init_Search_ISO_Folder_Name)"
		}
	...
	function Refresh_Prerequisite_Is_Satisfy
	{
		...
		if ($Script:Prerequisite_Is_Satisfy.Count -gt 0) {
			Get-Volume | Where-Object DriveType -eq 'CD-ROM' | ForEach-Object {
				Get-DiskImage -DevicePath $_.Path.trimend('\') -EA SilentlyContinue | ForEach-Object {

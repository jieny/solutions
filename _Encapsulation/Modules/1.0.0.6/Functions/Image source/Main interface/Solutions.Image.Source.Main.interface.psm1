...
Function Image_Select
{
		...
		if (Test-Path -Path $ImageFile -PathType leaf) {
			...
			try {
				...
				if ($MarkErrorMounted) {
					...
					$test_mount_folder = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Master)\$($ImageName)\Mount" -ErrorAction SilentlyContinue
	...
	$UIUnzip           = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "DiskTo" -ErrorAction SilentlyContinue) {
				...
				$UIUnzipPanel_Menu_Sources_Path.Text = Join-Path -Path $itemISO -ChildPath $Global:Init_Search_ISO_Folder_Name -ErrorAction SilentlyContinue
			}
	...
	$UIUnzipPanelOK    = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			$Full_New_Path = Join-Path -Path $UIUnzipPanel_To_New_Sources.Text -ChildPath $UIUnzipPanel_To_New_Path.Text -ErrorAction SilentlyContinue
		...
		@{
			Main = @(
				@{
					...
					Suffix        = "wim"
					Path          = Join-Path -Path $Global:Image_source -ChildPath "Sources" -ErrorAction SilentlyContinue
				}
			)
			Expand = @(
				@{
					...
					ImageFileName = "WinRe"
					Suffix        = "wim"
					Path          = Join-Path -Path $Global:Mount_To_Route -ChildPath "Install\Install\Mount\Windows\System32\Recovery" -ErrorAction SilentlyContinue

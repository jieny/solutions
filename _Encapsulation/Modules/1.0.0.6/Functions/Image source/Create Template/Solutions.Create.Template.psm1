							...
							ForEach ($DSType in $Mask_Desktop_Server) {
								ForEach ($ArchitectureNew in $Mask_Arch) {
									ForEach ($itemAR in $Mask_Add_Del) {
										$SaveFileTo = Join-Path -Path $Global:MainMasterFolder -ChildPath "$($DSType)\_Custom\$($item.main.ImageFileName)\$($item.main.ImageFileName)\Update\$($AIWB.Flag)\$($ArchitectureNew)\$($itemAR)" -ErrorAction SilentlyContinue
							...
							ForEach ($AIWB in $Mask_Install_WinRE_Boot) {
								if ($AIWB.Uid -eq $itemExpandNew.Uid) {
									ForEach ($DSType in $Mask_Desktop_Server) {
										ForEach ($ArchitectureNew in $Mask_Arch) {
											ForEach ($itemAR in $Mask_Add_Del) {
												$SaveFileTo = Join-Path -Path $Global:MainMasterFolder -ChildPath "$($DSType)\_Custom\$($item.main.ImageFileName)\$($itemExpandNew.ImageFileName)\Update\$($AIWB.Flag)\$($ArchitectureNew)\$($itemAR)" -ErrorAction SilentlyContinue

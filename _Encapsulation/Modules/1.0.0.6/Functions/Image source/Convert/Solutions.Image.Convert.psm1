...
Function Image_Convert_Process
{
	...
	$Install_wim = Join-Path -Path $Global:Image_source -ChildPath "Sources\install.wim" -ErrorAction SilentlyContinue
	$Install_swm = Join-Path -Path $Global:Image_source -ChildPath "Sources\install.swm" -ErrorAction SilentlyContinue
	$SearchImageSources = Join-Path -Path $Global:Image_source -ChildPath "Sources" -ErrorAction SilentlyContinue
	...
	switch ($Global:Queue_Convert_Tasks.Name)
	{
		...
		'ESDtoWIM'
		{
			...
			if ($Global:Queue_Convert_Tasks.Backup) {
				$SaveFileToEsd = Join-Path -Path $Global:MainMasterFolder -ChildPath "Backup\Install.wim\$($RandomGuid)\install.esd" -ErrorAction SilentlyContinue
				...
				$SaveFileToEsdTemp = Join-Path -Path $Global:MainMasterFolder -ChildPath "Backup\Install.wim\$($RandomGuid)" -ErrorAction SilentlyContinue
		...
		'WIMtoESD'
		{
			...
			if ($Global:Queue_Convert_Tasks.Backup) {
				$SaveFileToWim = Join-Path -Path $Global:MainMasterFolder -ChildPath "Backup\Install.wim\$($RandomGuid)\install.wim" -ErrorAction SilentlyContinue
				...
				$SaveFileToWimTemp = Join-Path -Path $Global:MainMasterFolder -ChildPath "Backup\Install.wim\$($RandomGuid)" -ErrorAction SilentlyContinue
		...
		'MergedSWM'
		{
			...
			$SaveFileToSwm = Join-Path -Path $Global:MainMasterFolder -ChildPath "Backup\Install.wim\$($RandomGuid)\install*.swm" -ErrorAction SilentlyContinue
			$SaveFileToSwmMatch = Join-Path -Path $Global:Image_source -ChildPath "Sources\*.swm" -ErrorAction SilentlyContinue
			$SaveBackToGuid = Join-Path -Path $Global:MainMasterFolder -ChildPath "Backup\Install.wim\$($RandomGuid)" -ErrorAction SilentlyContinue
			...
			try {
				Get-WindowsImage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Get.log" -ImagePath $Install_SWM -ErrorAction SilentlyContinue | ForEach-Object {
					...
					Write-Host "   $($lang.Rebuilding)".PadRight(28) -NoNewline
					$SaveFileToSwmFull = Join-Path -Path $Global:Image_source -ChildPath "Sources\install*.swm" -ErrorAction SilentlyContinue

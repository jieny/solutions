	...
	if (Test-Path -Path $ImageFile -PathType leaf) {
		...
		try {
			...
			Get-WindowsImage -Mounted -ErrorAction SilentlyContinue | ForEach-Object {
				$test_mount_folder = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($ImageMaster)\$($ImageName)\Mount" -ErrorAction SilentlyContinue
...
function Get_Mount_To_Temp
{
	...
	$Temp_New_Temp_Path = Join-Path -Path $Global:Mount_To_RouteTemp -ChildPath $RandomGuid -ErrorAction SilentlyContinue

			...
			if ($FlagCheckISOName) {
				...
				if (Test-Path -Path $ISOTestFolderMain -PathType Container) {
					<#
						...
						.创建目录成功，创建一个空文件
					#>
					$test_iso_Save_Temp = Join-Path -Path $Global:ISOSaveToFolder -ChildPath "$($RandomGuid)\writetest-$($RandomGuid)" -ErrorAction SilentlyContinue
...
Function ISO_Local_Language_Calc
{
	...
	ForEach ($itemRegion in $Region) {
		...
		$TestFolderRegion = Join-Path -Path $Global:Image_source -ChildPath "Sources\$($itemRegion.Region)" -ErrorAction SilentlyContinue
		...
		if (Test-Path -Path $TestFolderRegion -PathType Container) {
			...
			foreach ($NewSearchItem in $NewSearchFileRule) {
				$TestFolderRegionNewItem = Join-Path -Path $Global:Image_source -ChildPath "Sources\$($itemRegion.Region)\$($NewSearchItem)" -ErrorAction SilentlyContinue
...
#					write-host "   $($TestFolderRegionNewItem), $($NewSearchFileCalc)" -ForegroundColor Yellow
	...
	ForEach ($item in $Global:Image_Rule) {
		if ($item.Main.Suffix -eq "wim") {
			$SearchMainPath = @(
				Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($item.Main.ImageFileName)\Language\Add" -ErrorAction SilentlyContinue
				"$($Global:Image_source)_Custom\$($item.Main.ImageFileName)\$($item.Main.ImageFileName)\Language\Add"
				Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\$($item.Main.ImageFileName)\$($item.Main.ImageFileName)\Language" -ErrorAction SilentlyContinue
			)
			...
			if ($item.Expand.Count -gt 0) {
				ForEach ($Expand in $item.Expand) {
					$SearchExpandPath = @(
						Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($Expand.ImageFileName)\Language\Add" -ErrorAction SilentlyContinue
						"$($Global:Image_source)_Custom\$($item.Main.ImageFileName)\$($Expand.ImageFileName)\Language\Add"
						Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\$($item.Main.ImageFileName)\$($Expand.ImageFileName)\Language" -ErrorAction SilentlyContinue
					)
	...
	if ($Global:Queue_ISO) {
		...
		if (Test-Path -Path $Global:ISOSaveToFolder -PathType Container) {
			...
			<#
				...
				.处理：绕过 TPM
			#>
			Write-Host "`n   $($lang.BypassTPM)" -ForegroundColor Yellow
			Write-host "   $('-' * 80)"
			if ($Global:BypassTPM) {

Function Feature_Enabled_UI
{
	...
	$TestFolderMountCurrent = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue
...
Function Feature_Enabled_Process
{
	...
	if ($Temp_Queue_Is_Feature_Enable_Custom_Select.count -gt 0) {
		...
		ForEach ($item in $Temp_Queue_Is_Feature_Enable_Custom_Select) {
			...
			$TestFolderMountRoute = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Windows" -ErrorAction SilentlyContinue
			$TestFolderMountCurrent = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue
			$TestFolderMountSxs = Join-Path -Path $Global:Image_source -ChildPath "Sources\sxs" -ErrorAction SilentlyContinue

...
Function Feature_More_UI
{
	...
	$SearchFolderRule = @(
		Join-Path -Path $Global:Image_source -ChildPath "History\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Report" -ErrorAction SilentlyContinue
		Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Report" -ErrorAction SilentlyContinue

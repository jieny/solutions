...
Function Language_Menu
{
	...
	$LanguageRepair_ISO_Path = Join-Path -Path $Global:Image_source -ChildPath "Sources" -ErrorAction SilentlyContinue
	$LanguageRepair_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Sources" -ErrorAction SilentlyContinue
		$SearchFolderRule = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Language\Repair" -ErrorAction SilentlyContinue
...
Function Language_Sync_To_ISO_Process
{
	...
	ForEach ($itemRegion in $Region) {
		<#
			.同步已挂载的语言包到 ISO 安装程序
		#>
		$TestFolderRegion = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\sources\$($itemRegion.Region)" -ErrorAction SilentlyContinue
...
{
	$TestFolderMount_To_Route = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue

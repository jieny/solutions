...
Function Language_Add_UI
{
	...
	$SearchFolderRule = @(
		Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Language\Add" -ErrorAction SilentlyContinue
	...
	$Search_Folder_Multistage_Rule = @(
		Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Language" -ErrorAction SilentlyContinue
	)

...
Function Update_Add_UI
{
	...
	#>
	$SearchFolderRule = @(
		Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\$($ArchitectureNew)\Add" -ErrorAction SilentlyContinue
	...
	$Search_Folder_Multistage_Rule = @(
		Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update" -ErrorAction SilentlyContinue
	)
	...
	Function Autopilot_Update_Add_UI_Save
	{
		$UI_Main_Error.Text = ""
		...
		if ($Temp_Get_Select_Queue.Count -gt 0) {
			...
			Refres_Event_Tasks_Update_Add_UI
	...
	Function Refres_Event_Tasks_Update_Add_UI
	{
	...
	$UI_Main_Save      = New-Object system.Windows.Forms.Button -Property @{
		...
		Text           = $lang.Save
		add_Click      = {
			if (Autopilot_Update_Add_UI_Save) {
...
Function Autopilot_Update_Add_UI_Import
{
	param (
		$Tasks
	)
			...
			"Auto" {
				...
				if ($New_Tasks_Assign_Auto_Schome.Count -gt 0) {
					...
					if ($Is_Valid_New_Custom_Path.count -gt 0) {
						...
						Update_Add_UI -Autopilot $New_Event
					} else {
						Write-Host "   $($lang.NoWork)" -ForegroundColor Red
					}
				} else {

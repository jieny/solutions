...
Function Update_Delete_UI
{
	...
	#>
	$SearchFolderRule = @(
		Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\$($ArchitectureNew)\Del" -ErrorAction SilentlyContinue
	...
	Function Autopilot_Update_Delete_UI_Save
	{
		$UI_Main_Error.Text = ""
	...
	$UI_Main_Save      = New-Object system.Windows.Forms.Button -Property @{
		...
		Text           = $lang.Save
		add_Click      = {
			if (Autopilot_Update_Delete_UI_Save) {
...
Function Autopilot_Update_Delete_UI_Import
{
	param (
		$Tasks
	)
			...
			"Auto" {
				...
				if ($New_Tasks_Assign_Auto_Schome.Count -gt 0) {
					...
					if ($Is_Valid_New_Custom_Path.count -gt 0) {
						...
						Update_Delete_UI -Autopilot $New_Event
					} else {
						Write-Host "   $($lang.NoWork)" -ForegroundColor Red
					}
				} else {

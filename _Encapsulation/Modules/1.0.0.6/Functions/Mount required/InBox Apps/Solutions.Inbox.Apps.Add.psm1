...
Function InBox_Apps_Add_UI
{
	...
	#>
	$SearchTypeInboxApps = @(
		Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\InBox_Apps" -ErrorAction SilentlyContinue
		Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\InBox_Apps\Add" -ErrorAction SilentlyContinue
	)
		...
		add_Click      = {
			if (InBox_Apps_Check_Customize -RuleNaming -SelectApps -AddSources) {
				...
				if ($Temp_Assign_Task_InBox_Apps.Count -gt 0) {
					...
					$test_mount_folder_Current = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Report\InBox.Apps" -ErrorAction SilentlyContinue

...
Function InBox_Apps_Mark_UI
{
	...
	$UI_Main_Report    = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			#>
			if ([string]::IsNullOrEmpty($GUIISOCustomizeName.Text)) {
				$UI_Main_Mask_Report_Save_To.Text = Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\InBox_Apps\Report.$($RandomGuid).csv" -ErrorAction SilentlyContinue
				$UI_Main_Mask_Report_Sources_Path.Text = Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\InBox_Apps" -ErrorAction SilentlyContinue
			} else {
				<#
					.获取是否有添加源
				#>
...
Function InBox_Apps_LIPs_Clean_Process
{
	...
	$test_mount_folder_Current = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue

...
Function Feature_More
{
	...
	Write-host "   $('-' * 80)"
	$TestWIMFile = Join-Path -Path $Global:Image_source -ChildPath "Sources" -ErrorAction SilentlyContinue
	Write-host "   $($TestWIMFile)" -ForegroundColor Yellow

	$TestWIMFile = Join-Path -Path $Global:Image_source -ChildPath "Sources\Boot.wim" -ErrorAction SilentlyContinue
	...
	$TestWIMFile = Join-Path -Path $Global:Image_source -ChildPath "Sources\Install.wim" -ErrorAction SilentlyContinue
	if (Test-Path -Path $TestWIMFile -PathType Leaf) {
	...
	$TestWIMFile = Join-Path -Path $Global:Image_source -ChildPath "Sources\Install.swm" -ErrorAction SilentlyContinue
	if (Test-Path -Path $TestWIMFile -PathType Leaf) {
	...
	$TestWIMFile = Join-Path -Path $Global:Image_source -ChildPath "Sources\Install.esd" -ErrorAction SilentlyContinue
	if (Test-Path -Path $TestWIMFile -PathType Leaf) {
	...
	$TestNewPathMount = Join-Path -Path $Global:Mount_To_Route -ChildPath "Install\Install\Mount" -ErrorAction SilentlyContinue
	$TestNewPathMountRouter = Join-Path -Path $Global:Mount_To_Route -ChildPath "Install\Install\Mount\Windows\System32\Recovery\WinRE.wim" -ErrorAction SilentlyContinue
	...
	switch (Read-Host "`n   $($lang.PleaseChoose)")
	{
		...
		'1' {
			Image_Get_Detailed -Filename $(Join-Path -Path $Global:Image_source -ChildPath "Sources\Boot.wim" -ErrorAction SilentlyContinue) -View
			Get_Next
		...
		'2' {
			Image_Get_Detailed -Filename $(Join-Path -Path $Global:Image_source -ChildPath "Sources\Install.wim" -ErrorAction SilentlyContinue) -View
		...
		'3' {
			Image_Get_Detailed -Filename $(Join-Path -Path $Global:Image_source -ChildPath "Sources\Install.swm" -ErrorAction SilentlyContinue) -View
		...
		'4' {
			Image_Get_Detailed -Filename $(Join-Path -Path $Global:Image_source -ChildPath "Sources\Install.esd" -ErrorAction SilentlyContinue) -View
		...
		'5' {
			if (Image_Is_Select_Install) {
				Image_Get_Detailed -Filename $(Join-Path -Path $Global:Mount_To_Route -ChildPath "Install\Install\Mount\Windows\System32\Recovery\winre.wim" -ErrorAction SilentlyContinue) -View
				Get_Next
			} else {
...
Function Image_Get_Detailed
{
	...
	if (Test-Path -Path $Filename -PathType Leaf) {
		...
		$test_create_report_folder_Current = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Report" -ErrorAction SilentlyContinue
		$test_create_report_folder_route = Join-Path -Path $Global:Mount_To_Route -ChildPath "Report" -ErrorAction SilentlyContinue

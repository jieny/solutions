...
Function Event_Need_Mount_Global_Variable
{
	...
	#>
		<#
			.本地语言体验包（LXPs）,标记
		#>
		New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_$($Master)_$($ImageFileName)" -Value $False -Force

		<#
			.本地语言体验包（LXPs），标记，用户自选项
		#>
		New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Custom_Select_$($Master)_$($ImageFileName)" -Value @() -Force

		<#
			.本地语言体验包（LXPs）,标记, 用户选择主要来源路径
		#>
#		New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Mark_UI_Select_Sources_$($Master)_$($ImageFileName)" -Value @() -Force
...
#			New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Add_Not_Select_$($Master)_$($ImageFileName)" -Value @() -Force
		...
		#>
			<#
				.自动根据映像版本分配应用程序
			#>
			New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Auto_Assign_Editions_$($Master)_$($ImageFileName)" $true -Force

			<#
				.自动从所有磁盘搜索缺少的软件包
			#>
			New-Variable -Scope global -Name "Queue_Is_InBox_Apps_Missing_Packer_$($Master)_$($ImageFileName)" $true -Force

			<#
				.安装 InBox Apps 应用时，允许自动组合依赖包
			#>
			New-Variable -Scope global -Name "Queue_Is_InBox_Apps_DependencyPackage_$($Master)_$($ImageFileName)" $true -Force

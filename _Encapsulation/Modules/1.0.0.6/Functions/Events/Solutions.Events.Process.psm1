...
Function Event_Process_Task_Need_Mount
{
	...
	if ((Get-Variable -Scope global -Name "Queue_Is_Language_Add_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value) {
		...
		if ((Get-Variable -Scope global -Name "Queue_Is_Setup_Fix_Missing_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value) {
			...
			$SearchFolderRule = @(
				Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Language\Repair" -ErrorAction SilentlyContinue
			...
			foreach ($item in $SearchFolderRule) {
				...
				if (Test-Path -Path $item -PathType Container) {
					$test_mount_Sources = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Sources" -ErrorAction SilentlyContinue
	...
	<#
		...
		.第一步：添加本地语言体验包 (LXPs)
	#>
	$Host.UI.RawUI.WindowTitle = "$($lang.Event_Primary_Key): $($Global:Primary_Key_Image.UID), $($lang.MountedIndex): $($Get_Index_Now), $($lang.StepOne)"
	...
	#>
	$Host.UI.RawUI.WindowTitle = "$($lang.Event_Primary_Key): $($Global:Primary_Key_Image.UID), $($lang.MountedIndex): $($Get_Index_Now), $($lang.StepTwo): $($lang.AddTo)"
	...
	<#
		...
		.第三步：删除本地语言体验包 (LXPs)
	#>
	$Host.UI.RawUI.WindowTitle = "$($lang.Event_Primary_Key): $($Global:Primary_Key_Image.UID), $($lang.MountedIndex): $($Get_Index_Now), $($lang.Del): $($lang.LocalExperiencePackTips)"
	...
	<#
		...
		.第四步：仅添加本地语言体验包 (LXPs)
	#>
	$Host.UI.RawUI.WindowTitle = "$($lang.Event_Primary_Key): $($Global:Primary_Key_Image.UID), $($lang.MountedIndex): $($Get_Index_Now), $($lang.LocalExperiencePackTips): $($lang.Update)"
	...
	#>
	$Host.UI.RawUI.WindowTitle = "$($lang.Event_Primary_Key): $($Global:Primary_Key_Image.UID), $($lang.MountedIndex): $($Get_Index_Now), $($lang.InboxAppsOfflineDel)"
	...
	#>
	$Host.UI.RawUI.WindowTitle = "$($lang.Event_Primary_Key): $($Global:Primary_Key_Image.UID), $($lang.MountedIndex): $($Get_Index_Now), $($lang.UWPOptimize)"
	...
	if ((Get-Variable -Scope global -Name "Queue_Is_Update_Curing_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value) {
		...
		$test_mount_Sources = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue
	...
	<#
		.获取预安装应用 UWP
	#>
	$Host.UI.RawUI.WindowTitle = "$($lang.Event_Primary_Key): $($Global:Primary_Key_Image.UID), $($lang.MountedIndex): $($Get_Index_Now), $($lang.GetImageUWP)"
				...
				if ($item.Expand.Count -gt 0) {
					...
					ForEach ($itemExpandNew in $item.Expand) {
						<#
							.初始化变量
						#>
						$Temp_Do_Not_Save_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($itemExpandNew.ImageFileName)\Mount" -ErrorAction SilentlyContinue
						...
						if ((Get-Variable -Scope global -Name "Mark_Is_Mount_$($item.Main.ImageFileName)_$($itemExpandNew.ImageFileName)").Value) {
							...
							if ((Get-Variable -Scope global -Name "Queue_Expand_Healthy_$($item.Main.ImageFileName)_$($itemExpandNew.ImageFileName)").Value) {
								<#
									.触发此事件强制结束
								#>
								$test_mount_Sources_Expand = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($itemExpandNew.ImageFileName)\Mount" -ErrorAction SilentlyContinue
...
#									New-Variable -Scope global -Name "Queue_Expand_Eject_Only_Save_$($item.Main.ImageFileName)_$($itemExpandNew.ImageFileName)" -Value $False -Force
	...
	if ($IsEjectAfterSaveNot) {
		...
		$Temp_Do_Not_Save_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue
		...
		if ($item.Main.Suffix -eq "wim") {
			if ($item.Expand.Count -gt 0) {
				...
				ForEach ($Expand in $item.Expand) {
					$Eject_Expand_Do_Not_Save_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($Expand.ImageFileName)\Mount" -ErrorAction SilentlyContinue
			...
			$Eject_Main_Do_Not_Save_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($item.Main.ImageFileName)\Mount" -ErrorAction SilentlyContinue

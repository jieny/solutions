...
Function Wimlib_Extract_And_Update
{
	...
	$Search_Folder_Multistage_Rule = @(
		...
		Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\Wimlib" -ErrorAction SilentlyContinue
		Join-Path -Path $Global:MainMasterFolder -ChildPath "$($Global:ImageType)\_Custom\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\Wimlib" -ErrorAction SilentlyContinue
	)
	...
	Function Refresh_Wimlib_Update_Mul_Sources
	{
		...
		$AutoSelect_Path_Rule = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\Wimlib" -ErrorAction SilentlyContinue
		...
		if ($item.Main.Suffix -eq "wim") {
			...
			if ($item.Main.Uid -eq $Global:Primary_Key_Image.Uid) {
				...
				if ($item.Expand.Count -gt 0) {
					...
					$UI_Main_Sync_To_Wim.ContextMenuStrip = $GUIGroupUWPAddMenu
					# 2222222222222222222222222222222222222222222222222222222222222

...
Function Other_Tasks_Delete_Folder_Yi
{
	...
	$Local_Regedit_File_System = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Yi" -ErrorAction SilentlyContinue
...
Function Other_Tasks_Add_Test_Folder
{
	...
	$Local_Regedit_File_System = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\$($RandomGuid)" -ErrorAction SilentlyContinue
...
Function Other_Tasks_TPM
{
	...
	$test_Mount_Folder = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue

	Write-Host "   $($test_Mount_Folder)"
	if (Test-Path -Path $test_Mount_Folder -PathType Container) {
		$Local_Regedit_File_System = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Windows\System32\Config\SYSTEM" -ErrorAction SilentlyContinue
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "RAMDisk_Volume_Label" -ErrorAction SilentlyContinue) {
		...
		Get-CimInstance -ClassName Win32_Volume -ErrorAction SilentlyContinue | Where-Object { -not ([string]::IsNullOrEmpty($_.DriveLetter) -or [string]::IsNullOrWhiteSpace($_.DriveLetter))} | ForEach-Object {
			if ($_.Label -eq $GetRegRAMDISKVolumeLabel) {
				...
				Write-host "$($_.DriveLetter) ( $($_.Label) )" -ForegroundColor Green

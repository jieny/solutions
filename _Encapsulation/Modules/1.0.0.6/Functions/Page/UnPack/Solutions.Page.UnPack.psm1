...
Function UnPack_Create_UI
{
	...
	$UI_Main_Create_ASC = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Create_ASC.Checked) {
				...
				Save_Dynamic -regkey "Solutions" -name "IsPGP_Unpack" -value "True" -String
			} else {
				$UI_Main_Create_ASC_Panel.Enabled = $False
				Save_Dynamic -regkey "Solutions" -name "IsPGP_Unpack" -value "False" -String
			}
		}
	}
	...
	$GUIUnPackBackup   = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			if (Test-Path -Path "$($TempFolderUnPack)\writetest-$($RandomTempFileGuid)" -PathType Leaf) {
				...
				UnPack_Compression_Process -Path "$(Convert-Path "$($PSScriptRoot)\..\..\..\..\..\.." -ErrorAction SilentlyContinue)"
				UnPack_Create_SHA256_GPG -Path $TempFolderUnPack
				UnPack_Move_To -OldPath $TempFolderUnPack -NewPath $UnPackSaveTo
				Update_Create_Version -SaveTo $UnPackSaveTo -CurrentVersion (Get-Module -Name Solutions).Version.ToString() -LowVer $((Get-Module -Name Solutions).PrivateData.PSData.MinimumVersion)
		...
		add_Click      = {
			...
			if ($GUIUnPackSources.Checked) {
				...
				ForEach ($item in $SearchUnPack) {
					$Script:SelectFolderList += Join-Path -Path $Global:MainMasterFolder -ChildPath $item -ErrorAction SilentlyContinue
				}
	...
	ForEach ($item in $SearchUnPack) {
		$CheckBox         = New-Object System.Windows.Forms.CheckBox -Property @{
			Height        = 40
			Width         = 310
			Text          = Join-Path -Path $Global:MainMasterFolder -ChildPath $item -ErrorAction SilentlyContinue
			Checked       = $True
			add_Click      = {
				$UI_Main_Error.Text = ""
				$UI_Main_Error_Icon.Image = $null
	...
	if (Test-Path -Path $Verify_Install_Path -PathType leaf) {
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "IsPGP_Unpack" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "IsPGP_Unpack" -ErrorAction SilentlyContinue) {
				"True" {

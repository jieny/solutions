...
Function Solutions_Create_UI
{
	...
	Function Solutions_Unattend_Refresh_Status
	{
		...
		$SaveFileTo = Join-Path -Path $Global:Image_source -ChildPath "Sources\EI.CFG" -ErrorAction SilentlyContinue
...
"@ | Out-File -FilePath $SaveFileTo -Encoding Ascii -ErrorAction SilentlyContinue
	...
	$SolutionsOfficeChangeCfgClick = {
		...
		$OfficePathNew = Join-Path -Path $Global:Image_source -ChildPath "Sources\`$OEM$\`$1\$($GUISolutionsCustomizeName.Text)" -ErrorAction SilentlyContinue
	...
	if ($ISO) {
		...
		if (-not $Global:AutopilotMode -xor $Global:EventQueueMode) {
			...
			if (Image_Is_Select_IAB) {
				...
				$TestFolderMountRoute = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue
...
Function Solutions_Generate_Prerequisite
{
	...
	if ($ISO) {
		$Script:init_To_GPS = "ISO"
		$Script:CopySolutionsToRoot    = Join-Path -Path $Global:Image_source -ChildPath "Sources\`$OEM$\`$1" -ErrorAction SilentlyContinue
		$Script:CopySolutionsToWindows = Join-Path -Path $Global:Image_source -ChildPath "Sources\`$OEM$\`$$" -ErrorAction SilentlyContinue
	...
	if ($Mount) {
		if (Image_Is_Select_IAB) {
			$Script:init_To_GPS = "$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)"
			$Script:CopySolutionsToRoot = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue
			$Script:CopySolutionsToWindows = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Windows" -ErrorAction SilentlyContinue
	...
	if ((Get-Variable -Scope global -Name "SolutionsUnattend_$($Script:init_To_GPS)" -ErrorAction SilentlyContinue).Value) {
		...
		if ((Get-Variable -Scope global -Name "SolutionsCreateUnattendISOSources_$($Script:init_To_GPS)" -ErrorAction SilentlyContinue).Value) {
			$TempGetFolder = Join-Path -Path $Global:Image_source -ChildPath "Sources" -ErrorAction SilentlyContinue
		...
		if ((Get-Variable -Scope global -Name "SolutionsCreateUnattendISOSourcesOEM_$($Script:init_To_GPS)" -ErrorAction SilentlyContinue).Value) {
			$TempGetFolder = Join-Path -Path $Global:Image_source -ChildPath "Sources\`$OEM$\`$$\Panther" -ErrorAction SilentlyContinue
		...
		if ((Get-Variable -Scope global -Name "SolutionsCreateUnattendPanther_$($Script:init_To_GPS)" -ErrorAction SilentlyContinue).Value) {
			if (Image_Is_Select_IAB) {
				$TempGetFolder = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -ErrorAction SilentlyContinue

				if (Test-Path -Path $TempGetFolder -PathType Container 	-ErrorAction SilentlyContinue) {
					$TempGetFolderPanther = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Windows\Panther" -ErrorAction SilentlyContinue

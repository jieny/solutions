...
Function Image_Select
{
	...
	If (Test-Path -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom") {
		...
		New-Item "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($Global:Author)" -force -ErrorAction SilentlyContinue | Out-Null
		$InitPath = Join-Path -Path $([Environment]::GetFolderPath("Desktop")) -ChildPath "$($Global:Author).ps1"
		New-ItemProperty -LiteralPath "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($Global:Author)" -Name "Path" -Value $InitPath -PropertyType String -force -ErrorAction SilentlyContinue | Out-Null
	}
				...
				if ($ReadBackupJson.Version.Replace('.', '') -ge $((Get-Module -Name Solutions).PrivateData.PSData.API.MinimumVersion).Replace('.', '')) {
					if ($ReadBackupJson.API.Count -gt 0) {
						foreach ($item in $ReadBackupJson.API) {
							New-Item "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($item.Name)" -force -ErrorAction SilentlyContinue | Out-Null
							New-ItemProperty -LiteralPath "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($item.Name)" -Name "Path" -Value $item.Path -PropertyType String -force -ErrorAction SilentlyContinue | Out-Null
						}
	...
	Function Add_New_Rule_Name
	{
		...
		if (Verify_New_RuleName) {
			...
			New-Item "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($GUIImageSourceGroupAPI_Rule_Path.Text)" -force -ErrorAction SilentlyContinue | Out-Null
			New-ItemProperty -LiteralPath "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($GUIImageSourceGroupAPI_Rule_Path.Text)" -Name "Path" -Value $GUIImageSourceGroupAPI_New_Path.Text -PropertyType String -force -ErrorAction SilentlyContinue | Out-Null

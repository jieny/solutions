...
Function Image_Select
{
	...
	Function Image_Select_Refresh_Mount_Disk
	{
		...
		Get-Volume -ErrorAction SilentlyContinue | Where-Object { -not ([string]::IsNullOrEmpty($_.DriveLetter) -or [string]::IsNullOrWhiteSpace($_.DriveLetter))} | ForEach-Object {
			$RootDiskMaster = (Join_MainFolder -Path "$($_.DriveLetter):")

			if (Test_Available_Disk -Path $RootDiskMaster) {
				$RadioButton  = New-Object System.Windows.Forms.RadioButton -Property @{
					Height    = 40
					Width     = 415
					Text      = "$($_.FileSystemLabel) ( $($RootDiskMaster) )"
					Tag       = $RootDiskMaster
				...
				if ($MarkRAMDISK_Is_Check) {
					if ($_.FileSystemLabel -eq $CustomRAMDISKLabel) {
				...
				if ($GUIImageSourceGroupMountChangeExclude.Checked) {
					if ($_.FileSystemType -eq "ReFS") {
						$RadioButton.Text = "$($_.FileSystemLabel) ( $($RootDiskMaster) ), $($lang.ReFS_Exclude)"
						$RadioButton.Enabled = $False
	...
	Function Refresh_Click_Image_Sources
	{
				...
				if ($_.Checked) {
					...
					#>
						...
						<#
							.注册表：已保存所有配置
						#>
						if (Test-Path -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)" -PathType Container) {
		...
		$UI_Main_Error.Text = "$($lang.Ok_Go_To): $($lang.Autopilot), $($lang.OnDemandPlanTask)"
	...
	$GUIImageSourceGroupMountFromDelete = New-Object system.Windows.Forms.LinkLabel -Property @{
				...
				if (Test-Path $GUIImageSourceGroupMountFromPath.Text -PathType Container) {
					...
					if ($GUIImageSourceGroupMountFromDelete_Custom.Enabled) {
						if ($GUIImageSourceGroupMountFromDelete_Custom.Checked) {
							if (Test-Path $GUIImageSourceGroupMountFromDelete_Custom_Path.Text -PathType Container) {
		...
		$GUIImageSourceGroupMountFromDelete_Custom_Path = New-Object system.Windows.Forms.Label -Property @{
			autoSize       = 1
			Text           = ""
			Padding        = "53,0,0,0"
			Margin         = "0,0,0,20"
	...
	$UIUnzip           = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "DiskTo" -ErrorAction SilentlyContinue) {
				$UIUnzipPanel_Menu_Sources_Path.Text = "$(Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "DiskTo" -ErrorAction SilentlyContinue)\$($Global:Init_Search_ISO_Folder_Name)"
			}
	...
	$CompressionType = [Collections.ArrayList]@(
		[pscustomobject]@{ CompressionType = ""; Lang = $lang.Ok_Go_To_No; }
		[pscustomobject]@{ CompressionType = "Event_Assign_Task_Customize_Autopilot"; Lang = $lang.Autopilot; }
		[pscustomobject]@{ CompressionType = "Event_Assign_Task_Customize"; Lang = $lang.OnDemandPlanTask; }
	)

	$UI_Main_To.Datasource = $CompressionType
	$UI_Main_To.ValueMember = "CompressionType"
			...
			$UI_Main_Select_Sources.Controls | ForEach-Object {
				if ($_ -is [System.Windows.Forms.RadioButton]) {
					...
					if ($_.Checked) {
						...
						Write-host "   $('-' * 80)"
						if ([string]::IsNullOrEmpty($UI_Main_To.SelectedItem.CompressionType)) {
							write-host "   $($lang.Ok_Go_To_No)" -ForegroundColor Red

						} else {
							write-host "   $($UI_Main_To.SelectedItem.Lang)" -ForegroundColor Green
							Invoke-Expression -Command $UI_Main_To.SelectedItem.CompressionType
						}

						...
						if ($_ -is [System.Windows.Forms.RadioButton]) {
							if ($_.Enabled) {
								if ($_.Checked) {
									...
									Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Package" -name "$($Script:init_To_GPS)_SolutionsPacker" -value $_.Text
								}
							}
						}
					}
...
#							KMSkeys
		...
		if ($SolutionsEngine.Checked) {
			if ([string]::IsNullOrEmpty($GroupMainVerList.Text)) {
				...
				<#
					.允许首次自动更新
				#>
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Create\Mark" -name "$($Script:init_To_GPS)_Auto_Update" -value "False"
				if ($EngineChkUpdate.Enabled) {
					if ($EngineChkUpdate.Checked) {
						Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Create\Mark" -name "$($Script:init_To_GPS)_Auto_Update" -value "True"
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Create\Mark" -Name "$($Script:init_To_GPS)_Auto_Update" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Create\Mark" -Name "$($Script:init_To_GPS)_Auto_Update" -ErrorAction SilentlyContinue) {
	...
	<#
		.软件包合集
	#>
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Package" -Name "$($Script:init_To_GPS)_SolutionsPacker" -ErrorAction SilentlyContinue) {
		$GetTempSelectPacker = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Package" -Name "$($Script:init_To_GPS)_SolutionsPacker" -ErrorAction SilentlyContinue

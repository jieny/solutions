...
Function Update_Delete_UI
{
	...
	<#
		初始化全局变量
	#>
	$Script:Temp_Select_Update_Del_Folder = @()

	$SearchFolderRule = @(
		"$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\$($ArchitectureNew)\Del"
	...
	if (-not $Global:EventQueueMode) {
		Logo -Title "$($lang.CUpdate): $($lang.Del)"
	...
	Function Update_Del_Refresh_Sourcs
	{
		...
		if ($Script:Temp_Select_Update_Del_Folder.count -gt 0) {
			ForEach ($item in $Script:Temp_Select_Update_Del_Folder) {
				$InitLength = $item.Length
				if ($InitLength -lt $InitCharacterLength) { $InitLength = $InitCharacterLength }
				...
				if (Test-Path -Path $filename -PathType Container) {
					...
					$UI_Main_Rule.Controls | ForEach-Object {
					    if ($_ -is [System.Windows.Forms.CheckBox]) {
					        $Get_Temp_Select_Update_Del_Folder += $_.Text
					    }
					}
					...
					if ($Get_Temp_Select_Update_Del_Folder -Contains $filename) {
						...
						$Script:Temp_Select_Update_Del_Folder += $filename
						Update_Del_Refresh_Sourcs
					}
				} else {
					$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
		...
		add_Click      = {
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				if ($Get_Temp_Select_Update_Del_Folder -Contains $FolderBrowser.SelectedPath) {
					...
					$Script:Temp_Select_Update_Del_Folder += $FolderBrowser.SelectedPath
					Update_Del_Refresh_Sourcs
				}
			} else {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
	...
	$UI_Main_OK        = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			$Temp_Queue_Update_Delete_Select = @()
				...
				if ($_ -is [System.Windows.Forms.CheckBox]) {
					if ($_.Enabled) {
						if ($_.Checked) {
							...
							$Temp_Queue_Update_Delete_Select += $_.Tag
						}
					}
				}
			}

			if ($Temp_Queue_Update_Delete_Select.Count -gt 0) {
				$UI_Main.Hide()
				...
				New-Variable -Scope global -Name "Queue_Is_Update_Del_Custom_Select_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $Temp_Queue_Update_Delete_Select -Force
...
{
	if (-not $Global:EventQueueMode) {
		$Host.UI.RawUI.WindowTitle = "$($lang.Update): $($lang.Del)"
	}
		...
		ForEach ($item in $Temp_Assign_Task_Select) {
			Get-ChildItem $item -Recurse -Include ($Global:Search_KB_File_Type) -ErrorAction SilentlyContinue | ForEach-Object {
				if (Test-Path -Path $_.FullName -PathType Leaf) {
					...
					try {
						Remove-WindowsPackage -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -PackagePath $_.FullName -ErrorAction SilentlyContinue | Out-Null

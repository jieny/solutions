...
Function Verify_Available_Size
{
	...
	Get-CimInstance -Class Win32_LogicalDisk -ErrorAction SilentlyContinue | Where-Object { ((Join_MainFolder -Path $Disk) -eq (Join_MainFolder -Path $_.DeviceID)) } | ForEach-Object {
		if ($_.FreeSpace -gt (Convert_Size -From GB -To Bytes $Size)) {
			return $True
		} else {
			return $false
		}
					...
					switch ($_.MountStatus) {
						"Invalid" {
							...
							if (-not $Silent) {
								...
								if (Test-Path "$($Global:Mount_To_Route)\$($ImageMaster)\$($ImageName)\Mount" -PathType Container -ErrorAction SilentlyContinue) {

...
Function Drive_Add_UI
{
	...
	<#
		.初始化数组：选择新目录
	#>
	$Script:TempSelectDriveAddFolderQueue = @()

	$SearchFolderRule = @(
		"$($Global:MainMasterFolder)\$($Global:ImageType)\_Custom\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Drive\\$($ArchitectureNew)Add"
		"$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Drive\Add"
	...
	if (-not $Global:EventQueueMode) {
		Logo -Title "$($lang.Drive): $($lang.AddTo)"
	...
	Function Drive_Add_Refresh_Sources
	{
		...
		if ($Script:TempSelectDriveAddFolderQueue.count -gt 0) {
			ForEach ($item in $Script:TempSelectDriveAddFolderQueue) {
				$InitLength = $item.Length
				if ($InitLength -lt $InitCharacterLength) { $InitLength = $InitCharacterLength }
			...
			foreach ($filename in $_.Data.GetData([Windows.Forms.DataFormats]::FileDrop)) {
				if (Test-Path -Path $filename -PathType Container) {
					...
					if ($Get_Temp_Select_Update_Add_Folder -Contains $filename) {
						...
						$Script:TempSelectDriveAddFolderQueue += $filename
						Drive_Add_Refresh_Sources
					}
				} else {
					$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
		...
		add_Click      = {
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				if ($Get_Temp_Select_Update_Add_Folder -Contains $FolderBrowser.SelectedPath) {
					...
					$Script:TempSelectDriveAddFolderQueue += $FolderBrowser.SelectedPath
					Drive_Add_Refresh_Sources
				}
			} else {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
	...
	$UI_Main_OK        = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			if ($Temp_Queue_Drive_Add_Select.Count -gt 0) {
				...
				New-Variable -Scope global -Name "Queue_Is_Drive_Add_Custom_Select_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -Value $Temp_Queue_Drive_Add_Select -Force

				ForEach ($item in $Temp_Queue_Drive_Add_Select) {
					Write-Host "   $($item)"
				}
...
{
	if (-not $Global:EventQueueMode) {
		$Host.UI.RawUI.WindowTitle = "$($lang.Drive): $($lang.AddTo)"
	}

	$Temp_Queue_Drive_Add_Custom_Select = (Get-Variable -Scope global -Name "Queue_Is_Drive_Add_Custom_Select_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value
	if ($Temp_Queue_Drive_Add_Custom_Select.count -gt 0) {
		Write-Host "   $($lang.AddSources)" -ForegroundColor Yellow
		Write-host "   $('-' * 80)"
		ForEach ($item in $Temp_Queue_Drive_Add_Custom_Select) {
				...
				Get-ChildItem -Path $item -Recurse -include "*.inf" | Where-Object {
					if (Test-Path $_.FullName -PathType Leaf) {
						if ((Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -ErrorAction SilentlyContinue).'ShowCommand' -eq "True") {
							...
							Write-host "   $($lang.Developers_Mode_Location)32" -ForegroundColor Green
							Write-host "   $('-' * 80)"
						...
						Add-WindowsDriver -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Add.log" -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -Driver "$($_.FullName)" -Recurse -ForceUnsigned -ErrorAction SilentlyContinue | Out-Null
...
Function Image_Get_Installed_Drive
{
	...
	if (Test-Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -PathType Container) {
		...
		try {
			if ((Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -ErrorAction SilentlyContinue).'ShowCommand' -eq "True") {
				Write-Host "`n   $($lang.Command)" -ForegroundColor Green
				Write-host "`n   $($lang.Developers_Mode_Location)33" -ForegroundColor Green
				Write-host "   $('-' * 77)"
			...
			Get-WindowsDriver -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Get.log" -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -All | ForEach-Object {

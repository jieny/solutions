...
{
	$Script:TempSelectUWPAddFolderQueue = @()
	...
	if (-not $Global:EventQueueMode) {
		Logo -Title "$($lang.StepTwo)$($lang.AddTo)"
		...
		if ($_.Data.GetDataPresent([Windows.Forms.DataFormats]::FileDrop)) {
			foreach ($filename in $_.Data.GetData([Windows.Forms.DataFormats]::FileDrop)) {
				if (Test-Path -Path $filename -PathType Container) {
					...
					if ($TempSelectUWPAddFolderQueue -Contains $filename) {
						$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
	...
	$UI_Main_Mask_Dependencies_Fix_Select = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			ForEach ($item in $Global:InitlNoSelectUWPDependency) {
	...
	$UI_Main_Select_Folder = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			$UI_Main_Select_Sources.Controls | ForEach-Object {
				if ($_ -is [System.Windows.Forms.CheckBox]) {
					$Script:TempSelectUWPAddFolderQueue += $_.Text
				}
			}
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				if ($Script:TempSelectUWPAddFolderQueue -Contains $FolderBrowser.SelectedPath) {
					$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
					$UI_Main_Error.Text = $lang.Existed
				} else {
					<#
					...
					$InitLength = $FolderBrowser.SelectedPath.Length
					if ($InitLength -lt $InitCharacterLength) { $InitLength = $InitCharacterLength }
		...
		add_Click      = {
			...
			if (InBox_Apps_Check_Customize -RuleNaming -SelectApps -AddSources) {
				if ($UI_Main_Mask_Dependencies_DoNot_Tips.Checked) {
					...
					if ($Global:InitlNoSelectUWPDependency.count -gt 0) {
						$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
						$UI_Main_Error.Text = "$($lang.RuleFindError -f $Global:InitlNoSelectUWPDependency.count)"
						return
					}
				}
				...
				<#
					...
					.自动从所有磁盘搜索缺少的软件包
				#>
				Write-Host "`n   $($lang.UWPAutoMissingPacker)"
				if ($UI_Main_Add_Missing_Apps.Enabled) {
					if ($UI_Main_Add_Missing_Apps.Checked) {
						Write-Host "   $($lang.Operable)" -ForegroundColor Green
						$Script:InboxMissingPacker = $True
					} else {
						$Script:InboxMissingPacker = $False
...
Function InBox_Apps_Check_Customize
{
	...
	<#
		.需要的依赖项，未选择
	#>
	$Global:InitlNoSelectUWPDependency = @()
			...
			ForEach ($Item in $Script:InBoxAppxUserNoSelect) {
				ForEach ($WildCard in $Script:DependencyPackageSelect) {
					if ($item -like $WildCard) {
						$Global:InitlNoSelectUWPDependency += $item
					}
				}
			}

			$UI_Main_Mask_Dependencies_Results.Text += "`n`n$($lang.RuleDependenciesUnselected) ( $($Global:InitlNoSelectUWPDependency.Count) $($lang.EventManagerCount) )`n"
			if ($Global:InitlNoSelectUWPDependency.count -gt 0) {
				ForEach ($Item in $Global:InitlNoSelectUWPDependency) {
					$UI_Main_Mask_Dependencies_Results.Text += "   $($item)`n"
				}

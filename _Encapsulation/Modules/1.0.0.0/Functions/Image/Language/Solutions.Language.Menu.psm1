...
Function Language_Sync_To_ISO_Process
{
	...
	ForEach ($itemRegion in $Region) {
		<#
			.同步已挂载的语言包到 ISO 安装程序
		#>
		if (Test-Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\sources\$($itemRegion.Region)" -PathType Container) {

...
Function Language_Add_UI
{
	...
	$SearchFolderRule = @(
		"$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Language\Add"
	...
	if (-not $Global:EventQueueMode) {
		Logo -Title "$($lang.Language): $($lang.AddTo)"
		...
		if (-not (Image_Is_Select_IAB)) {
			Write-Host "`n   $($lang.Language): $($lang.AddTo)"
			Write-host "   $('-' * 80)"
		...
		if ($UI_Main_Auto_Sync_Suggestions.Enabled) {
			if ($UI_Main_Auto_Sync_Suggestions.Checked) {
				...
				if ($MarkCheckIsLangSelect) {
					...
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Sync_Sources_Lang" -value "True" -String

					$UI_Main_Setup_Fix_Missing.Checked = $True
					$UI_Main_Setup_Fix_Missing.Enabled = $True
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Sync_Sources_Lang_Fix" -value "True" -String

					$UI_Main_Language_Ini_Rebuild.Checked = $True
					$UI_Main_Language_Ini_Rebuild.Enabled = $True
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Language_Ini_Rebuild" -value "True" -String
				} else {
	...
	$UI_Main_Is_Order_Rule_Lang = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Is_Order_Rule_Lang.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Order_Rule_Lang" -value "True" -String
				$UI_Main_Is_Match_installed_List.Enabled = $True
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Order_Rule_Lang" -value "False" -String
	...
	$UI_Main_Is_Match_installed_List = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Is_Match_installed_List.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Match_installed_List" -value "True" -String
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Match_installed_List" -value "False" -String
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Match_installed_List" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Match_installed_List" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Setup_Fix_Missing = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($This.Checked) {
				...
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Sync_Sources_Lang_Fix" -value "False" -String
			}
		}
	}
	...
	$UI_Main_Lang_Sync_To_Sources = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($This.Checked) {
				...
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Sync_Sources_Lang" -value "False" -String
			}
		}
	}
	...
	$UI_Main_Language_Ini_Rebuild = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($This.Checked) {
				...
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Is_Language_Ini_Rebuild" -value "False" -String
			}
		}
	}
	...
	$UI_Main_OK        = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			if ($Temp_Select_Add_New_Language_Sources.Count -gt 0) {
				...
				Write-host "   $('-' * 80)"
				ForEach ($item in $Temp_Select_Add_New_Language_Sources) {
				...
				<#
					.安装语言包时，从已安装列表里通配
				#>
				Write-Host "`n   $($lang.Match_installed_List)"
				if ($UI_Main_Is_Order_Rule_Lang.Enabled) {
					if ($UI_Main_Is_Order_Rule_Lang.Checked) {
				...
				Write-Host "`n   $($lang.Setup_Fix_Missing): $($lang.Mounted)"
	...
	if (Image_Is_Select_Boot) {
		...
		$UI_Main_Lang_Sync_To_Sources.Enabled = $True
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Sync_Sources_Lang" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Sync_Sources_Lang" -ErrorAction SilentlyContinue) {
				"True" {
					$UI_Main_Lang_Sync_To_Sources.Checked = $True
				}
				"False" {
		...
		$UI_Main_Setup_Fix_Missing.Enabled = $True
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Sync_Sources_Lang_Fix" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Sync_Sources_Lang_Fix" -ErrorAction SilentlyContinue) {
				"True" {
					$UI_Main_Setup_Fix_Missing.Checked = $True
				}
			}
		...
		$UI_Main_Language_Ini_Rebuild.Enabled = $True
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Language_Ini_Rebuild" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Is_Language_Ini_Rebuild" -ErrorAction SilentlyContinue) {
				"True" {
					$UI_Main_Language_Ini_Rebuild.Checked = $True
				}
			}
...
{
	if (-not $Global:EventQueueMode) {
		$Host.UI.RawUI.WindowTitle = "$($lang.Language): $($lang.AddTo)"
	}
		...
		if ((Get-Variable -Scope global -Name "Queue_Is_Language_Add_Category_$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)" -ErrorAction SilentlyContinue).Value) {
			...
			ForEach ($item in $Temp_Language_Add_Custom_Select) {
				ForEach ($itemRegion in $Region) {
					...
					if (Test-Path $FullNewPath -PathType Container -ErrorAction SilentlyContinue) {
...
Function Language_Add_File_Type_Process
{
	...
	if (($Script:Init_Exclude_File) -contains $FileName) {
		...
		if ((Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -ErrorAction SilentlyContinue).'ShowCommand' -eq "True") {
			...
			Write-host "   $($lang.Developers_Mode_Location)62" -ForegroundColor Green
			Write-host "   $('-' * 80)"
			write-host "   Add-WindowsPackage -Path ""$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"" -PackagePath ""$($FileName)""" -ForegroundColor Green
			Write-host "   $('-' * 80)`n"
		}

		Write-Host "   $($lang.AddTo)".PadRight(28) -NoNewline
		try {
			Add-WindowsPackage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Add.log" -Path "$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount" -PackagePath "$($FileName)" -ErrorAction SilentlyContinue | Out-Null
...
Function Language_Add_Rule_Process
{
	...
	if ($Script:Init_Folder_All_File.Count -gt 0) {
		Write-Host "`n   $($lang.LXPsWaitAdd) ( $($Script:Init_Folder_All_File.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
		Write-host "   $('-' * 80)"
...
Function Language_Rule_Add_group
{
	...
	<#
		.OCR
	#>
	Write-Host "   $($lang.OCR) ( $($Script:Init_File_Type_OCR.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	...
	<#
		.Handwriting
	#>
	Write-Host "`n   $($lang.Handwriting) ( $($Script:Init_File_Type_Handwriting.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	...
	<#
		.TextToSpeech
	#>
	Write-Host "`n   $($lang.TextToSpeech) ( $($Script:Init_File_Type_TextToSpeech.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	...
	<#
		.Speech
	#>
	Write-Host "`n   $($lang.Speech) ( $($Script:Init_File_Type_Speech.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	...
	<#
		.Features_On_Demand
	#>
	Write-Host "`n   $($lang.Features_On_Demand) ( $($Script:Init_File_Type_Features_On_Demand.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	if ($Script:Init_File_Type_Features_On_Demand.Count -gt 0) {
		...
		Write-host "`n   $($lang.ExcludeItem) ( $($Script:Init_Exclude_File.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
		Write-host "   $('-' * 80)"
...
Function Language_Rule_Add_View_And_Process
{
	...
	<#
		.1 = 字体
	#>
	Write-Host "`n   $($lang.Fonts) ( $($Script:Init_File_Type_Fonts.Count) $($lang.EventManagerCount) )" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	if ($Script:Init_File_Type_Fonts.Count -gt 0) {
		ForEach ($item in $Script:Init_File_Type_Fonts) {
			Write-Host "   $([IO.Path]::GetFileName($item.FileName))" -ForegroundColor Green
	...
	<#
		.2 = 基本
	#>
	Write-Host "`n   $($lang.Basic) ( $($Script:Init_File_Type_Basic.Count) $($lang.EventManagerCount) )" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	...
	<#
		.3 = OCR
	#>
	Write-Host "`n   $($lang.OCR) ( $($Script:Init_File_Type_OCR.Count) $($lang.EventManagerCount) )" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	if ($Script:Init_File_Type_OCR.Count -gt 0) {
	...
	<#
		.7 = 零售演示体验
	#>
	Write-Host "`n   $($lang.Retail) ( $($Script:Init_File_Type_Retail.Count) $($lang.EventManagerCount) )" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	...
	<#
		.9 = 特定包
	#>
	Write-Host "`n   $($lang.RegionSpecific) ( $($Script:Init_File_Type_Other_Region_Specific.Count) $($lang.EventManagerCount) )" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"
	...
	<#
		.10 = 语言包其它
	#>
	Write-Host "`n   $($lang.UnassignedLangFiles) ( $($Script:Init_Folder_All_File_Exclude.Count) $($lang.EventManagerCount) )" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"

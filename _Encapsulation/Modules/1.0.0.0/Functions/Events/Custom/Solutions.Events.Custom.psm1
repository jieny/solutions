...
Function Event_Assign_Task_Customize
{
	...
	#>
	#region 循环处理所有挂载的
#	$Global:Queue_Assign_Full.Main | Out-gridview
	ForEach ($WildCard in $Global:Queue_Assign_Full) {
		...
		if ($Global:Queue_Assign_Full.Count -gt 0) {
			...
			if ($WildCard.Main.UI.Count -gt 0) {
				...
				Write-Host "`n   $($lang.User_Interaction): $($lang.OnDemandPlanTask) ( $($WildCard.Main.UI.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
				Write-host "   $('-' * 80)"
				...
				if ((Get-Variable -Scope global -Name "Mark_Is_Mount_$($WildCard.Main.ImageFileName)_$($WildCard.Main.ImageFileName)").Value) {
					...
					if ($Temp_Queue_Process_Image_Select_Pending.Count -gt 0) {
						...
						Write-Host "`n   $($lang.SelectFromError)$($lang.MountedIndexSelect) ( $($WildCard.Main.Uid) )" -ForegroundColor Red
					}
		...
		if ($Global:Queue_Assign_Full.Count -gt 0) {
			...
			if ($WildCard.Expand.UI.Count -gt 0) {
				...
				ForEach ($Expand in $WildCard.Expand) {
					...
					Write-Host "`n   $($lang.User_Interaction): $($lang.OnDemandPlanTask) ( $($Expand.UI.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
					Write-host "   $('-' * 80)"
					...
					if (Test-Path -Path "$($Expand.Path)\$($Expand.ImageFileName).$($Expand.Suffix)" -PathType Leaf) {
						...
						if (Verify_Is_Current_Same) {
							...
							if ($Temp_Queue_Process_Image_Select_Pending.Count -gt 0) {
								...
								Write-Host "`n   $($lang.SelectFromError)$($lang.MountedIndexSelect) ( $($Expand.Uid) )" -ForegroundColor Red
							}
						...
						$Temp_Queue_Process_Image_Select_Pending = (Get-Variable -Scope global -Name "Queue_Process_Image_Select_Pending_$($WildCard.Main.ImageFileName)_$($WildCard.Main.ImageFileName)" -ErrorAction SilentlyContinue).Value
						if ($Temp_Queue_Process_Image_Select_Pending.Count -gt 0) {
							Write-Host "`n   $($lang.Choose) $($lang.SelectSettingImage)" -ForegroundColor Yellow
							Write-host "   $('-' * 80)"
	...
	<#
		.初始化时间：所有任务
	#>
	$Script:AllTasksTimeStart = Get-Date -Format "yyyy/MM/dd HH:mm:ss"
	...
	Write-host " $($Script:AllTasksTimeStart -f "yyyy/MM/dd HH:mm:ss")" -ForegroundColor Yellow
				...
				if ($WildCard.Expand.Count -gt 0) {
					ForEach ($ExpandWildcard in $WildCard.Expand) {
						<#
							.判断是否排除运行
						#>
						if (($Global:Extension_Has_been_Run) -Contains $ShowExpandItem.Uid) {
			...
			if ((Get-Variable -Scope global -Name "Mark_Is_Mount_$($WildCard.Main.ImageFileName)_$($WildCard.Main.ImageFileName)").Value) {
				...
				if ($WildCard.Expand.Count -gt 0) {
					ForEach ($itemExpandNew in $WildCard.Expand) {
						...
						Image_Queue_Wimlib_Process_Wim_Main -NewUid $itemExpandNew.Uid -NewMaster $WildCard.Main.ImageFileName -NewImageFileName $itemExpandNew.ImageFileName -MasterFile "$($WildCard.Main.Path)\$($WildCard.Main.ImageFileName).$($WildCard.Main.Suffix)"
...
#		$Local_Wim_Update_Folder_Sources = "$($Global:Mount_To_Route)\$($MainUid)\$($MainUid)\Update\Wimlib\$($RandomGuid)"
		...
		if (Test-Path $Local_Wim_Update_Full_Path -PathType Leaf) {
			if ($Global:Developers_Mode) {
...
Function Run_Expand
{
	...
	if (Event_Assign_Task_Verify -Mount) {
		...
		if (Verify_Is_Current_Same) {
			...
			if (Test-Path "$($NewExpand.Path)\$($NewExpand.ImageFileName).$($NewExpand.Suffix)" -PathType Leaf) {
				Write-Host "`n   $($lang.YesWork)" -ForegroundColor Yellow
				Write-host "   $('-' * 80)"
				...
				if ($Temp_Queue_Process_Image_Select_Pending.Count -gt 0) {
					...
					Write-Host "`n   $($lang.SelectFromError)$($lang.MountedIndexSelect) ( $($NewMain.ImageFileName);$($NewExpand.ImageFileName); )" -ForegroundColor Red
				}
				...
				switch ($Get_Current_Process_Type_Temp) {
					...
					"Popup" {
						...
						Write-Host "   $($lang.Choose) $($lang.SelectSettingImage)" -ForegroundColor Yellow
						Write-host "   $('-' * 80)"
						if ($Temp_Queue_Process_Image_Select_Pending.Count -gt 0) {
							ForEach ($itemBB in $Temp_Queue_Process_Image_Select_Pending) {
								Write-Host "   $($lang.Wim_Image_Name): " -noNewline

					...
					$Group_Image_Sources_Console.controls.AddRange((
						$GUIImageSelectFunctionNeedMount,
							...
							$GUIImageSelectFunctionUWP,
								...
								$GUIImageSelectFunctionUWP_Remove,
				...
				if ($UI_Main_Is_Select_Wim.Checked) {
					...
					if ($Global:Queue_Assign_Full.Count -gt 0) {
						...
						ForEach ($item in $Global:Queue_Assign_Full) {
							...
							Write-Host "`n      $($lang.User_Interaction): $($lang.OnDemandPlanTask) ( $($item.Main.UI.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
							Write-host "      $('-' * 77)"
							...
							if ($item.Expand.Count -gt 0) {
								...
								ForEach ($itemExpand in $item.Expand) {
									...
									Write-Host "`n         $($lang.User_Interaction): $($lang.OnDemandPlanTask) ( $($itemExpand.UI.Count) $($lang.EventManagerCount)" -ForegroundColor Yellow
									Write-host "         $('-' * 74)"

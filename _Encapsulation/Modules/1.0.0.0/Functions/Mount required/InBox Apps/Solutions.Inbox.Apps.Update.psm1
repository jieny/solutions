...
Function InBox_Apps_Update_UI
{
	...
	Function Refres_Event_Tasks_InBox_Apps_Update_UI
	{
	...
	$UI_Main_Event_Clear_Click = {
		...
		Refres_Event_Tasks_InBox_Apps_Update_UI

		$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Success.ico")
		$UI_Main_Error.Text = "$($lang.EventManagerCurrentClear), $($lang.Done)"
	}

	Function Autopilot_InBox_Apps_Update_UI_Save
	{
		$UI_Main_Error.Text = ""
	...
	Function Refresh_Add_InBox_Apps_New_Sources
	{
		...
		if (([string]::IsNullOrEmpty($Temp_Assign_Task_Select))) {
			...
			if (Test-Path -Path "$($Temp_Assign_Task_Select)\LocalExperiencePack" -PathType Container) {
				...
				if (Test-Path $Temp_Assign_Task_Select -PathType Container) {
	...
	$UI_Main_Save      = New-Object system.Windows.Forms.Button -Property @{
		...
		Text           = $lang.Save
		add_Click      = {
			if (Autopilot_InBox_Apps_Update_UI_Save) {
	...
	if ($Autopilot) {
		...
		if (Test-Path $Autopilot.Sources -PathType Container) {
...
Function InBox_Apps_LIPs_Add_Process
{
	if (-not $Global:EventQueueMode) {
		$Host.UI.RawUI.WindowTitle = "$($lang.LocalExperiencePackTips): $($lang.Update)"
	}

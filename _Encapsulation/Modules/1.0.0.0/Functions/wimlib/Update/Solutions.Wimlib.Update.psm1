...
Function Wimlib_Extract_And_Update
{
	...
	$Search_Folder_Multistage_Rule = @(
		...
		"$($Global:Mount_To_Route)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\Wimlib"
		"$($Global:MainMasterFolder)\$($Global:ImageType)\_Custom\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\Wimlib"
	)
...
Function Image_Queue_Wimlib_Process_Wim_Main
{
	...
	if ($Temp_Expand_Rule.Count -gt 0) {
		Write-host "`n   $($lang.Event_Allow_Update_Rule) ( $($Temp_Expand_Rule.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
		Write-host "   $('-' * 80)"
		...
		if ($Temp_Queue_Is_Update_Rule) {
			Write-Host "`n   $($lang.Event_Assign_Expand) ( $($Temp_Expand_Rule.Count) ) $($lang.EventManagerCount)" -ForegroundColor Yellow
			Write-host "   $('-' * 80)"

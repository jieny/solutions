...
Function Image_Convert_UI
{
	...
	$UI_Main_OK        = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			if ([string]::IsNullOrEmpty($Solutions_Office_Select_New.Text)) {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\Assets\icon\Error.ico")
				$UI_Main_Error.Text = "$($lang.SelectFromError)$($lang.CompressionType)"
				return
			}
			...
			if ($GUIImageConvertToWIM.Checked) {
				<#
					.转换前重建
				#>
				Write-Host "`n   $($lang.Rebuild)" -ForegroundColor Green
			...
			if ($Global:Queue_Convert_Tasks.Count -gt 0) {
				$UI_Main.Hide()
...
Function Image_Convert_Process
{
	...
	switch ($Global:Queue_Convert_Tasks.Name)
	{
		...
		'ESDtoWIM'
		{
			...
			Write-Host "   $($Global:Image_source)\sources\install.esd`n"
			if ($Global:Queue_Convert_Tasks.Backup) {
				Write-Host "`n   $($lang.ConvertBackup)"
				Write-Host "   $($Global:Image_source)\sources\install.esd"
				Write-Host "   >> "
				...
				Copy-Item $Install_ESD -Destination "$($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)\install.esd" -ErrorAction SilentlyContinue
				Image_Convert_Create_Info_Process -Sources $Install_ESD -SaveTo "$($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)"
			...
			if (Test-Path $Install_wim -PathType leaf) {
				...
				if ($Global:Queue_Convert_Tasks.Split.IsSplit) {
					Write-Host "   $($lang.Operable)" -ForegroundColor Green
					Get-ChildItem -Path "$($Global:Image_source)\sources" -Recurse -include "*.swm" | ForEach-Object {
						Write-Host "   $($lang.Del) ( $($_.Fullname) )`n" -ForegroundColor Green
						Remove-Item -Path $_.Fullname -ErrorAction SilentlyContinue
					}

					Write-Host "`n   $($Global:Image_source)\sources\install.wim" -ForegroundColor Green
					if (Test-Path $Install_wim -PathType leaf) {
						Write-Host "   $($lang.Operable)" -ForegroundColor Green
		...
		'WIMtoESD'
		{
			...
			if ($Global:Queue_Convert_Tasks.Backup) {
				...
				Copy-Item $Install_wim -Destination "$($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)\install.wim" -ErrorAction SilentlyContinue
				Image_Convert_Create_Info_Process -Sources $Install_wim -SaveTo "$($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)"
				Write-Host "   $($lang.Done)" -ForegroundColor Green
			}
		...
		'SplitWim'
		{
			Write-Host "`n   $($lang.Converting -f "Wim", "Swm")"
			Write-Host "   $($Install_wim)`n"
			
			if ($Global:Queue_Convert_Tasks.Backup) {
				Write-Host "`n   $($lang.ConvertBackup)"
				Write-Host "   $($Global:Image_source)\sources\install*.swm"
				Write-Host "   >> "
				Write-Host "   $($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)\install*.swm"

				Check_Folder -chkpath "$($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)"

				Copy-Item "$($Global:Image_source)\sources\*.swm" -Destination "$($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)" -ErrorAction SilentlyContinue
				Image_Convert_Create_Info_Process -Sources "$($Install_SWM)" -SaveTo "$($Global:MainMasterFolder)\Backup\Install.wim\$($RandomGuid)"
			...
			if ($Global:Queue_Convert_Tasks.Split.IsSplit) {
				...
				if (Test-Path $Install_wim -PathType leaf) {
					...
					Write-Host $Global:Queue_Convert_Tasks.Split.Size

					try {
						Split-WindowsImage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Split.log" -ImagePath "$($Global:Image_source)\sources\install.wim" -SplitImagePath "$($Global:Image_source)\sources\install.swm" -FileSize "$($Global:Queue_Convert_Tasks.Split.Size)" -CheckIntegrity
					} catch {
						Write-host $lang.ConvertChk
						Write-host "   $($Install_wim)"
						Write-Host "   $($_)" -ForegroundColor Yellow
		...
		'MergedSWM'
		{
			...
			Write-Host "   $($Global:Image_source)\sources\install.swm`n"
			...
			try {
				Get-WindowsImage -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Get.log" -ImagePath $Install_SWM -ErrorAction SilentlyContinue | ForEach-Object {
					...
					dism /ScratchDir:"""$(Get_Mount_To_Temp)""" /LogPath:"$(Get_Mount_To_Logs)\Export.log" /export-image /SourceImageFile:"""$($Install_SWM)""" /swmfile:"""$($Global:Image_source)\sources\install*.swm""" /SourceIndex:"""$($_.ImageIndex)""" /DestinationImageFile:"""$($Install_wim)""" /Compress:"""$($Global:Queue_Convert_Tasks.Compression)""" /CheckIntegrity

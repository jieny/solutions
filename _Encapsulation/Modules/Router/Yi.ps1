...
{
	Write-host "`n   $($Script:Lang.CleanupLogs)" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"	

	$TempPaths = @(
		$env:Temp
		"$($env:SystemRoot)\Logs\DISM"
		Join-Path -Path $(Convert-Path -Path "$($PSScriptRoot)\..\.." -ErrorAction SilentlyContinue) -ChildPath "Logs" -ErrorAction SilentlyContinue
	)
...
{
	Write-host "`n   $($Script:Lang.CleanupDisk)" -ForegroundColor Yellow
	Write-host "   $('-' * 80)"	
...
Function Help
{
   ...
   ---------------------------------------------------------------------------
      ...
      2    $($PSscript.BaseName) -Remove  | $($Script:Lang.RemoveRoute)
   ...
   $($Script:Lang.Repair)
     ...
     13    $($PSscript.BaseName) -FixBad  | $($Script:Lang.Clear_Bad_Mount)

   $($Script:Lang.Remove)
     22    $($PSscript.BaseName) -Logs    | $($Script:Lang.CleanupLogs)
     23    $($PSscript.BaseName) -Disk    | $($Script:Lang.CleanupDisk)

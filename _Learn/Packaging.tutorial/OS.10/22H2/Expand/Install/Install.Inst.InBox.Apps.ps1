        ...
        if($Allow_Install_App -contains $Rule.Name) {
            ...
            if ($ISO -eq "Auto") {
                Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue | ForEach-Object {
                    $AppPath = Join-Path -Path $_.Root -ChildPath "$($NewArch)\$($Rule.File)" -ErrorAction SilentlyContinue
                    $LicensePath = Join-Path -Path $_.Root -ChildPath "$($NewArch)\$($Rule.License)" -ErrorAction SilentlyContinue

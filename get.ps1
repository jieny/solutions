...
Function Installation_interface_UI
{
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Author)\Solutions\Get" -Name "InstlTo" -ErrorAction SilentlyContinue) {
		$UI_Main_Save_To_Path.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Author)\Solutions\Get" -Name "InstlTo" -ErrorAction SilentlyContinue
	} else {
	...
	$OKGoToNew = [Collections.ArrayList]@(
		...
		[pscustomobject]@{ Path = "Unpack";  Lang = $lang.OK_Go_To_Bak; }
	...
	#>
		...
		if (-not [string]::IsNullOrEmpty($GoTo)) {
			switch ($GoTo) {
				...
				"Unpack" {
					$UI_Main_To.SelectedIndex = $UI_Main_To.FindString($lang.OK_Go_To_Bak)
				}
				"No" {
		...
		if ($AddRouter) {
			...
			if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Author)\Solutions\Get" -Name "Env" -ErrorAction SilentlyContinue) {
				switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Author)\Solutions\Get" -Name "Env" -ErrorAction SilentlyContinue) {
		...
		if ($AddTakeOwnership) {
			...
			if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Author)\Solutions\Get" -Name "TakeOwnership" -ErrorAction SilentlyContinue) {
				switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Author)\Solutions\Get" -Name "TakeOwnership" -ErrorAction SilentlyContinue) {
	...
	if (Test-Path -Path $NewFilePath -PathType leaf) {
		...
		if (TestArchive -Path $NewFilePath) {
			...
			if ($UI_Main_Adv_Env.Checked) {
				...
				$Route_PS = Join-Path -Path $UI_Main_Save_To_Path.Text -ChildPath "_Encapsulation\Modules\Router\Yi.ps1" -ErrorAction SilentlyContinue
				if (Test-Path -Path $Route_PS -PathType leaf) {
					Start-Process "powershell" -ArgumentList "-ExecutionPolicy ByPass -file ""$($Route_PS)"" -Add" -Wait -WindowStyle Minimized
					write-host $lang.Done -ForegroundColor Green
				} else {
					write-host $lang.Inoperable -ForegroundColor Red
				}
			...
			if ([string]::IsNullOrEmpty($UI_Main_To.SelectedItem.Path)) {
				...
				switch ($UI_Main_To.SelectedItem.Path) {
					"SIP" {
						Write-host "   $($lang.OK_Go_To_Sip)" -ForegroundColor Yellow
						$Solutions_PS = Join-Path -Path $UI_Main_Save_To_Path.Text -ChildPath "_Encapsulation\_Sip.ps1" -ErrorAction SilentlyContinue
					...
					"Tempate" {
						Write-host "   $($lang.OK_Go_To_Tempate)" -ForegroundColor Yellow
						$Solutions_PS = Join-Path -Path $UI_Main_Save_To_Path.Text -ChildPath "_Encapsulation\_Create.Template.ps1" -ErrorAction SilentlyContinue
						if (Test-Path -Path $Solutions_PS -PathType leaf) {
							write-host "   $($lang.Done)" -ForegroundColor Green
							Start-Process "powershell" -ArgumentList "-ExecutionPolicy ByPass -file ""$($Solutions_PS)""" -NoNewWindow
						} else {
					...
					"EUpdate" {
						Write-host "   $($lang.OK_Go_To_Engline_Update)" -ForegroundColor Yellow
						$Solutions_PS = Join-Path -Path $UI_Main_Save_To_Path.Text -ChildPath "_Encapsulation\_Create.Custom.Engine.upgrade.package.ps1" -ErrorAction SilentlyContinue
						if (Test-Path -Path $Solutions_PS -PathType leaf) {
							write-host "   $($lang.Done)" -ForegroundColor Green
							Start-Process "powershell" -ArgumentList "-ExecutionPolicy ByPass -file ""$($Solutions_PS)""" -NoNewWindow
						} else {
					...
					"Unpack" {
						Write-host "   $($lang.OK_Go_To_Bak)" -ForegroundColor Yellow
						$Solutions_PS = Join-Path -Path $UI_Main_Save_To_Path.Text -ChildPath "_Encapsulation\_Unpack.ps1" -ErrorAction SilentlyContinue
						if (Test-Path -Path $Solutions_PS -PathType leaf) {
							write-host "   $($lang.Done)" -ForegroundColor Green
							Start-Process "powershell" -ArgumentList "-ExecutionPolicy ByPass -file ""$($Solutions_PS)""" -NoNewWindow
						} else {
